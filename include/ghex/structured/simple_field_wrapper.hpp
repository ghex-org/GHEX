...
namespace gridtools {
    ...
    GT_FUNCTION array<T, D> operator+(array<T, D> a, const array<T, D>& b) {
    ...
    GT_FUNCTION typename std::common_type<T, U>::type dot(const array<T, D>& a, const array<U, D>& b) {
    ...
    namespace ghex {
        namespace structured {
            ...
            __global__ void pack_kernel(int size, const T* data, T* buffer, array<I, D> local_first,
                                        array<I, D> local_strides, array<S, D> byte_strides, array<I, D> offsets) {
            ...
            template<typename Layout, typename T, std::size_t D, typename I, typename S>
            __global__ void unpack_kernel(int size, T* data, const T* buffer, array<I, D> local_first,
                                          array<I, D> local_strides, array<S, D> byte_strides, array<I, D> offsets) {
                const auto index = blockIdx.x * blockDim.x + threadIdx.x;
                if (index < size) {
                    // compute local coordinate
            ...
            struct serialization {
                ...
                GT_FUNCTION_HOST static void pack(T* buffer, const IndexContainer& c, const T* m_data,
                                                  const Strides& m_byte_strides, const Array& m_offsets, void*) {
                    for (const auto& is : c) {
                        ::gridtools::ghex::detail::
                            for_loop_pointer_arithmetic<Dimension::value, Dimension::value, Layout>::apply(
                                ...
                                is.local().first(), is.local().last(), m_byte_strides, m_offsets);
                        buffer += is.size();
                    }
                }

                template<typename T, typename IndexContainer, typename Strides, typename Array>
                GT_FUNCTION_HOST static void unpack(const T* buffer, const IndexContainer& c, T* m_data,
                                                    const Strides& m_byte_strides, const Array& m_offsets, void*) {
                    for (const auto& is : c) {
                        ::gridtools::ghex::detail::
                            for_loop_pointer_arithmetic<Dimension::value, Dimension::value, Layout>::apply(
            ...
            struct serialization<gpu, Dimension, Layout> {
                template<typename T, typename IndexContainer, typename Strides, typename Array>
                GT_FUNCTION_HOST static void pack(T* buffer, const IndexContainer& c, const T* m_data,
                                                  const Strides& m_byte_strides, const Array& m_offsets, void* arg) {
     ...
     * @tparam Order permutation of the set {0,...,N-1} indicating storage layout (N-1 -> stride=1)*/
         ...
         * @param extents extent of the wrapped N-dimensional array (including buffer regions)*/
                ...
                GT_FUNCTION_HOST simple_field_wrapper(domain_id_type dom_id, value_type* data, const Array& offsets,
                                                      const Array& extents, device_id_type d_id = 0)
                : m_dom_id(dom_id)
                , m_data(data)
                , m_device_id(d_id) {
                ...
                GT_FUNCTION_HOST simple_field_wrapper(domain_id_type dom_id, value_type* data, const Array0& offsets,
                                                      const Array1& extents, const Array2& byte_strides,
                                                      device_id_type d_id = 0)
                : m_dom_id(dom_id)
                , m_data(data)
                , m_device_id(d_id) {
                    std::copy(offsets.begin(), offsets.end(), m_offsets.begin());
                ...
                template<typename Array>
                GT_FUNCTION_HOST simple_field_wrapper(domain_id_type dom_id, value_type* data, const Array& offsets,
                                                      const Array& extents, padding_256, device_id_type d_id = 0)
                : m_dom_id(dom_id)
                , m_data(data)
                , m_device_id(d_id) {
                    std::copy(offsets.begin(), offsets.end(), m_offsets.begin());
                    ...
                    const auto ext_1 =
                        m_extents[layout_map::template find<dimension::value - 1>()] * sizeof(value_type);
                    const std::size_t padding = ((ext_1 + 255) / 256) * 256 - ext_1;
                    // compute strides
                    detail::compute_strides<dimension::value>::template apply<layout_map, value_type>(
                        m_extents, m_byte_strides, padding);
         ...
         * @return reference to value */
                ...
                GT_FUNCTION value_type& operator()(Is&&... is) {
                    return *reinterpret_cast<T*>((char*)m_data +
                                                 dot(coordinate_type{is...} + m_offsets, m_byte_strides));
                }
                template<typename... Is>
                GT_FUNCTION const value_type& operator()(Is&&... is) const {
                    return *reinterpret_cast<const T*>((const char*)m_data +
                                                       dot(coordinate_type{is...} + m_offsets, m_byte_strides));
                }
                ...
                void unpack(const T* buffer, const IndexContainer& c, void* arg) {
                    serialization<Arch, dimension, layout_map>::unpack(buffer, c, m_data, m_byte_strides, m_offsets,
                                                                       arg);
                }
            };
        } // namespace structured
     ...
     * @return wrapped field*/
        ...
        structured::simple_field_wrapper<T, Arch, structured::domain_descriptor<DomainIdType, sizeof...(Order)>,
                                         Order...>
