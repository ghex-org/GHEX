...
class callback_communicator {
  ...
  public: // attach/detach
    ...
    void attach_send(future_type&& fut, message_type msg, rank_type dst, tag_type tag){
        m_sends.push_back(recv_element_type{ [] (message_type, rank_type, tag_type) {}, dst, tag, std::move(
            fut), std::move(msg) });
    }
    ...
    void attach_recv(future_type&& fut, message_type msg, rank_type src, tag_type tag){
        m_recvs.push_back(send_element_type{ [] (message_type, rank_type, tag_type) {}, src, tag, std::move(
            fut), std::move(msg) });
    }
