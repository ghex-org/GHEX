        ...
        namespace tl {
            ...
            class callback_communicator {
              ...
              public: // ctors
                ...
                : m_comm{typename communicator_type::traits{}}
                , m_alloc(alloc) {
                    m_sends.reserve(128);
                    m_recvs.reserve(128);
                }
              ...
              public: // send
                ...
                void send(const message_type& msg, rank_type dst, tag_type tag, CallBack&& cb) {
                    GHEX_CHECK_CALLBACK
                    auto& element = *m_sends.insert(
                        m_sends.end(), send_element_type{std::forward<CallBack>(cb), dst, tag, future_type{}, msg});
              ...
              public: // recieve
                ...
                void recv(message_type& msg, rank_type src, tag_type tag, CallBack&& cb) {
                    GHEX_CHECK_CALLBACK
                    auto& element = *m_recvs.insert(
                        m_recvs.end(), recv_element_type{std::forward<CallBack>(cb), src, tag, future_type{}, msg});
              ...
              public: // attach/detach
                ...
                void attach_send(future_type&& fut, message_type msg, rank_type dst, tag_type tag, CallBack&& cb) {
                    GHEX_CHECK_CALLBACK
                    m_sends.push_back(
                        send_element_type{std::forward<CallBack>(cb), dst, tag, std::move(fut), std::move(msg)});
                }
                ...
                void attach_send(future_type&& fut, message_type msg, rank_type dst, tag_type tag) {
                    m_sends.push_back(recv_element_type{[](message_type, rank_type, tag_type) {}, dst, tag,
                ...
                void attach_recv(future_type&& fut, message_type msg, rank_type src, tag_type tag, CallBack&& cb) {
                    GHEX_CHECK_CALLBACK
                    m_recvs.push_back(
                        send_element_type{std::forward<CallBack>(cb), src, tag, std::move(fut), std::move(msg)});
                }
                ...
                void attach_recv(future_type&& fut, message_type msg, rank_type src, tag_type tag) {
                    m_recvs.push_back(send_element_type{[](message_type, rank_type, tag_type) {}, src, tag,
                                                        std::move(fut), std::move(msg)});
                }
              ...
              private: // implementation
                ...
                boost::optional<std::pair<future_type, message_type>> detach(rank_type rank, tag_type tag, Deque& d) {
                    auto it = std::find_if(d.begin(), d.end(),
                                           [rank, tag](auto const& x) { return (x.m_rank == rank && x.m_tag == tag); });
                    if (it != d.end()) {
                        //auto cb =  std::move(it->m_cb);
                        auto fut = std::move(it->m_future);
                        auto msg = std::move(it->m_msg);
                        //d.erase(it);
                        if (it + 1 != d.end()) { *it = std::move(d.back()); }
                        d.resize(d.size() - 1);
