                ...
                class setup_communicator : public communicator_base {
                  ...
                  public:
                    ...
                    status recv(int source, int tag, T& value) {
                        MPI_Status status;
                        GHEX_CHECK_MPI_RESULT(MPI_Recv(reinterpret_cast<void*>(&value), sizeof(T), MPI_BYTE, source,
                                                       tag, *this, &status));
                        return {status};
                    }
                    ...
                    void send(int dest, int tag, const T* values, int n) {
                        GHEX_CHECK_MPI_RESULT(
                            MPI_Send(reinterpret_cast<const void*>(values), sizeof(T) * n, MPI_BYTE, dest, tag, *this));
                    }

                    template<typename T>
                    status recv(int source, int tag, T* values, int n) {
                        MPI_Status status;
                        GHEX_CHECK_MPI_RESULT(MPI_Recv(reinterpret_cast<void*>(values), sizeof(T) * n, MPI_BYTE, source,
                                                       tag, *this, &status));
                        return {status};
                    }
                    ...
                    future<std::vector<std::vector<T>>> all_gather(const std::vector<T>& payload,
                        ...
                        for (int neigh = 0; neigh < size(); ++neigh) {
                            ...
                            GHEX_CHECK_MPI_RESULT(MPI_Irecv(reinterpret_cast<void*>(res[neigh].data()),
                                                            sizeof(T) * sizes[neigh], MPI_BYTE, neigh, 99, *this,
                                                            &m_reqs.back().get()));
                        }
                        for (int neigh = 0; neigh < size(); ++neigh) {
                            m_reqs.push_back(handle_type{});
                            GHEX_CHECK_MPI_RESULT(MPI_Isend(reinterpret_cast<const void*>(payload.data()),
                                                            sizeof(T) * payload.size(), MPI_BYTE, neigh, 99, *this,
                                                            &m_reqs.back().get()));
                        }
