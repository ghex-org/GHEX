                ...
                struct communicator {
                    ...
                    [[nodiscard]] future<void> send(const MsgType& msg, rank_type dst, tag_type tag) {
                        ...
                        if (reinterpret_cast<std::uintptr_t>(ret) == UCS_OK) {
                            ...
                            return request{
                                request::data_type::construct(ret, m_recv_worker, m_send_worker, request_kind::send)};
                        } else {
                            // an error occurred
                            ...
                            [this, rtag, &msg, src, tag]() {
                                ...
                                if (!UCS_PTR_IS_ERR(ret)) {
                                    if (UCS_INPROGRESS != ucp_request_check_status(ret)) {
                                        ...
                                        return request{request::data_type::construct(ret, m_recv_worker, m_send_worker,
                                                                                     request_kind::recv)};
                                    }
                                } else {
                                    // an error occurred
                    ...
                    unsigned progress() {
                        ...
                        m_send_worker->m_parallel_context->thread_primitives().critical([this, &p]() {
                    ...
                    request_cb_type send(std::shared_ptr<Message>& shared_msg_ptr, rank_type dst, tag_type tag,
                                         CallBack&& callback) {
                    ...
                    request_cb_type send(shared_message_buffer<Alloc>& shared_msg, rank_type dst, tag_type tag,
                                         CallBack&& callback) {
                        GHEX_CHECK_CALLBACK_F(message_type, rank_type, tag_type)
                    ...
                    lvalue_func<Message&&> send(Message&& msg, rank_type dst, tag_type tag, CallBack&& callback) {
                        ...
                        return send(message_type{ref_message<V>{msg.data(), msg.size()}}, dst, tag,
                    ...
                    request_cb_type send(message_type&& msg, rank_type dst, tag_type tag, CallBack&& callback) {
                        ...
                        if (reinterpret_cast<std::uintptr_t>(ret) == UCS_OK) {
                            ...
                            auto req_ptr = request_cb_data_type::construct(
                                ret, m_send_worker, request_kind::send, std::move(msg), dst, tag,
                                std::forward<CallBack>(callback), std::make_shared<request_cb_state_type>(false));
                    ...
                    template<typename Message, typename CallBack>
                    request_cb_type recv(std::shared_ptr<Message>& shared_msg_ptr, rank_type src, tag_type tag,
                                         CallBack&& callback) {
                        GHEX_CHECK_CALLBACK_F(message_type, rank_type, tag_type)
                    ...
                    template<typename Alloc, typename CallBack>
                    request_cb_type recv(shared_message_buffer<Alloc>& shared_msg, rank_type src, tag_type tag,
                                         CallBack&& callback) {
                        GHEX_CHECK_CALLBACK_F(message_type, rank_type, tag_type)
                    ...
                    lvalue_func<Message&&> recv(Message&& msg, rank_type src, tag_type tag, CallBack&& callback) {
                        GHEX_CHECK_CALLBACK_F(message_type, rank_type, tag_type)
                        using V = typename std::remove_reference_t<Message>::value_type;
                        return recv(message_type{ref_message<V>{msg.data(), msg.size()}}, src, tag,
                                    std::forward<CallBack>(callback));
                    }
                    ...
                    inline static void recv_callback(void* __restrict ucx_req, ucs_status_t __restrict status,
                            ...
                            [this, rtag, &msg, src, tag, &callback]() {
                                ...
                                if (!UCS_PTR_IS_ERR(ret)) {
                                    if (UCS_INPROGRESS != ucp_request_check_status(ret)) {
                                        ...
                                        auto req_ptr = request_cb_data_type::construct(
                                            ret, m_recv_worker, request_kind::recv, std::move(msg), src, tag,
                                            std::forward<CallBack>(callback),
