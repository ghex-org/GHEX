        ...
        namespace tl {
            namespace ucx {
                ...
                struct address_db_mpi {
                    ...
                    , m_rank{[](MPI_Comm c) {
                        int r;
                        GHEX_CHECK_MPI_RESULT(MPI_Comm_rank(c, &r));
                        return r;
                    }(comm)}
                    , m_size{[](MPI_Comm c) {
                        int s;
                        GHEX_CHECK_MPI_RESULT(MPI_Comm_size(c, &s));
                        return s;
                    }(comm)} {}
                    ...
                    void init(const value_t& addr) {
                        ...
                        for (key_t r = 0; r < m_size; ++r) {
                            if (r == m_rank) {
                                ...
                                GHEX_CHECK_MPI_RESULT(MPI_Bcast(&size, sizeof(std::size_t), MPI_BYTE, r, m_mpi_comm));
                                ...
                                GHEX_CHECK_MPI_RESULT(MPI_Bcast(&size, sizeof(std::size_t), MPI_BYTE, r, m_mpi_comm));
                                value_t addr(size);
                                GHEX_CHECK_MPI_RESULT(MPI_Bcast(addr.data(), size, MPI_BYTE, r, m_mpi_comm));
