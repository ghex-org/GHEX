        ...
        namespace tl {
            ...
            public: // ctors
                ...
                , m_tokens(m_parallel_context.thread_primitives().size()) {
                    ...
                    GHEX_CHECK_UCX_RESULT(ucp_config_read(NULL, NULL, &config_ptr));
                    ...
                    // initialize UCP
                    GHEX_CHECK_UCX_RESULT(ucp_init(&context_params, config_ptr, &m_context.m_context));
                ...
                communicator_type get_serial_communicator() { return {&m_worker, &m_worker}; }
            ...
            namespace ucx {
                ...
                template<typename ThreadPrimitives>
                worker_t<ThreadPrimitives>::worker_t(transport_context_type* c, parallel_context_type* pc,
                                                     thread_token* t, ucs_thread_mode_t mode)
                : m_context{c}
                , m_parallel_context{pc}
                , m_token_ptr{t}
                , m_rank(c->rank())
                ...
                const endpoint_t& worker_t<ThreadPrimitives>::connect(rank_type rank) {
                    ...
                    if (auto addr_ptr = m_context->m_db.find(rank)) {
                        auto p =
                            m_endpoint_cache.insert(std::make_pair(rank, endpoint_t{rank, m_worker.get(), *addr_ptr}));
            ...
            struct context_factory<ucx_tag, ThreadPrimitives> {
                static std::unique_ptr<context<ucx_tag, ThreadPrimitives>> create(int num_threads, MPI_Comm mpi_comm) {
                    return std::make_unique<context<ucx_tag, ThreadPrimitives>>(num_threads, mpi_comm,
                                                                                ucx::address_db_mpi{mpi_comm});
                }
            };
        } // namespace tl
    }     // namespace ghex
