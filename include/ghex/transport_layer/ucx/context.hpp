...
struct transport_context<ucx_tag, ThreadPrimitives> {
  ...
  public: // ctors
    ...
    communicator_type get_communicator(const thread_token& t){
        if (!m_workers[t.id()]) {
            m_tokens[t.id()] = t;
            m_workers[t.id()] = std::make_unique<worker_type>(this, &m_parallel_context,
                    &m_tokens[t.id()], UCS_THREAD_MODE_SINGLE);
        }
...
template<typename ThreadPrimitives>
worker_t<ThreadPrimitives>::worker_t(transport_context_type* c, parallel_context_type* pc, thread_token* t,
