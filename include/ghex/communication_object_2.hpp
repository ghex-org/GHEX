    ...
    namespace ghex {
        ...
        class communication_object {
          ...
          public: // exchange a number of buffer_infos with Field = simple_field_wrapper (optimization for gpu below)
            ...
            template<typename Arch, typename T, int... Order>
            [[nodiscard]] std::enable_if_t<std::is_same<Arch, cpu>::value, handle_type> exchange_u(
                buffer_info_type<Arch, structured::simple_field_wrapper<
                                           T, Arch, structured::domain_descriptor<domain_id_type, sizeof...(Order)>,
                                           Order...>>* first,
                std::size_t length) {
          ...
          private: // implementation
            ...
            [[nodiscard]] handle_type exchange_impl(buffer_info_type<Arch, Field>* first, std::size_t length) {
                ...
                using test_t = pattern_container<communicator_type, grid_type, domain_id_type>;
                static_assert(
                    std::is_same<test_t, typename buffer_info_type<Arch, Field>::pattern_container_type>::value,
                    "patterns are not compatible with this communication object");
                if (m_valid) throw std::runtime_error("earlier exchange operation was not finished");
                m_valid = true;
          ...
          private: // allocation member functions
            ...
            void allocate(Memory& mem, const pattern_type& pattern, Field* field_ptr, domain_id_type dom_id,
                ...
                if (!pool) {
                    pool.reset(
                        new typename arch_traits<Arch>::pool_type{typename arch_traits<Arch>::basic_allocator_type{}});
                }
            ...
            template<typename Arch, typename ValueType, typename BufferType, typename Memory, typename Halos,
                     typename Function, typename DeviceIdType, typename Pool, typename Field = void>
            void allocate(Memory& memory, const Halos& halos, Function&& func, domain_id_type my_dom_id,
                ...
                for (const auto& p_id_c : halos) {
                    ...
                    if (it == memory.end()) {
                        it = memory
                                 .insert(
                                     std::make_pair(d_p, BufferType{remote_address, p_id_c.first.tag + tag_offset,
                                                                    arch_traits<Arch>::make_message(pool, device_id), 0,
                    ...
                    const auto padding =
                        ((prev_size + alignof(ValueType) - 1) / alignof(ValueType)) * alignof(ValueType) - prev_size;
