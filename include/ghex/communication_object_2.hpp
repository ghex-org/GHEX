...
class communication_object {
  ...
  public: // exchange arbitrary field-device-pattern combinations
    ...
    [[nodiscard]] handle_type exchange(buffer_info_type<Archs, Fields>... buffer_infos){
        ...
        static_assert(detail::test_eq_t<test_t, typename buffer_info_type<Archs,
            Fields>::pattern_container_type...>::value,
  ...
  public: // exchange a number of buffer_infos with Field = simple_field_wrapper (optimization for gpu below)
    ...
    [[nodiscard]] std::enable_if_t<std::is_same<Arch, gpu>::value, handle_type>exchange_u(buffer_info_type<Arch,
        structured::simple_field_wrapper<T, Arch, structured::domain_descriptor<domain_id_type, sizeof...(Order)>,
        Order...>>* first,
        std::size_t length){
    ...
    template<typename Arch, typename T, int... Order>
    [[nodiscard]] std::enable_if_t<std::is_same<Arch, cpu>::value, handle_type>exchange_u(buffer_info_type<Arch,
        structured::simple_field_wrapper<T, Arch, structured::domain_descriptor<domain_id_type, sizeof...(Order)>,
        Order...>>* first,
        std::size_t length){
  ...
  private: // allocation member functions
    ...
    template<typename Arch, typename ValueType, typename BufferType, typename Memory, typename Halos, typename Function,
    ...
    void allocate(Memory& memory, const Halos& halos, Function&& func, domain_id_type my_dom_id, DeviceIdType device_id,
        ...
        for (const auto& p_id_c : halos) {
            ...
            const auto padding = ((prev_size + alignof(ValueType) - 1) / alignof(ValueType)) * alignof(ValueType) -
                prev_size;
            it->second.field_infos.push_back(
                typename BufferType::field_info_type{ std::forward<Function>(
                    func), &p_id_c.second, prev_size + padding, field_ptr });
