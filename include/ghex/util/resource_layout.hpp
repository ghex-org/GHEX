 ...
 * 
 */
#ifndef INCLUDED_GHEX_UTIL_RESOURCE_LAYOUT_HPP
#define INCLUDED_GHEX_UTIL_RESOURCE_LAYOUT_HPP
...
struct dist_1D_generator<std::index_sequence<Is...>>
{
    ...
    static hierarchical_distribution<sizeof...(Js)> generate_1(const Dims& dims, std::index_sequence<Js...>) noexcept
    {
...
private:
    template<std::size_t... Is>
    static distribution_type make_dist(const dims_map_array_type& d, std::index_sequence<Is...>) noexcept
    {
...
private:
    size_type relative_resource(size_type idx, std::integral_constant<unsigned int, 0>) const noexcept
    {
        return idx - index<0>(idx)*std::get<0>(m_1D_dist).size();
    }

    template<unsigned int I>
    size_type relative_resource(size_type idx, std::integral_constant<unsigned int, I>) const noexcept
    {
        return relative_resource(idx, std::integral_constant<unsigned int, I-1>()) - index<I>(idx)*std::get<I>(m_1D_dist).size();
    }

    template<typename Array>
    void get_coord(const Array& indices, array_type& coord, std::integral_constant<unsigned int, 0>) const noexcept
    {
        constexpr unsigned int J = Levels-1;
        const auto level_coord = m_dims[J](indices[J]);
        for (unsigned i=0; i<D; ++i)
            coord[i] = level_coord[i] + coord[i]*m_dims[J].dims()[i];
    }

    template<typename Array, unsigned int I>
    void get_coord(const Array& indices, array_type& coord, std::integral_constant<unsigned int, I>) const noexcept
    {
...
#endif /* INCLUDED_GHEX_UTIL_RESOURCE_LAYOUT_HPP */
