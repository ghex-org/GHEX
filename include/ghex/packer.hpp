...
__global__ void pack_kernel_u(cuda::kernel_argument<kernel_args<T, Array, Field>, N> args){
    ...
    if (thread_index < size) {
        ...
        structured::detail::compute_coordinate<Array::size()>::template apply<layout_t>(arg.strides, local_coordinate,
            thread_index);
        // add offset
...
template<typename T, typename Array, typename Field, unsigned int N>
__global__ void unpack_kernel_u(cuda::kernel_argument<kernel_args<T, Array, Field>,
    N> args){
    using layout_t = typename Field::layout_map;
    const int thread_index = blockIdx.x * blockDim.x + threadIdx.x;
    const int data_lu_index = blockIdx.y;
                ...
                if (p1.second.size > 0u) {
                    ...
                    for (const auto& fb : p1.second.field_infos) {
                        ...
                        for (const auto& it_space_pair : *fb.index_container) {
                            ...
                            std::copy(&it_space_pair.local().last()[0],
                                &it_space_pair.local().last() [dimension::value], last.data());
                            array_t local_extents, local_strides;
                            for (std::size_t i = 0; i < dimension::value; ++i)
                                    local_extents[i] = 1 + last[i] - first[i];
                            structured::detail::compute_strides<dimension::value>::template apply<typename FieldType::layout_map>(
                                local_extents, local_strides);
                            args.push_back(arg_t{ size, buffer_address, first, local_strides,
                                                  *reinterpret_cast<FieldType*>(fb.field_ptr) });
                            buffer_address += size;
                        }
                    }
            ...
            [&block_size, &stream_ptrs, &args] (typename BufferMem::hook_type hook) {
                ...
                for (const auto& fb : hook->field_infos) {
                    ...
                    for (const auto& it_space_pair : *fb.index_container) {
                        ...
                        max_size = std::max(size, max_size);
                        array_t first, last;
                        std::copy(&it_space_pair.local().first()[0], &it_space_pair.local().first()[dimension::value],
                        first.data());
                        std::copy(&it_space_pair.local().last()[0], &it_space_pair.local().last() [dimension::value],
                        last.data());
                        array_t local_extents, local_strides;
                        for (std::size_t i = 0; i < dimension::value; ++i)
                                local_extents[i] = 1 + last[i] - first[i];
