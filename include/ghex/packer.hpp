    ...
    namespace ghex {
        ...
        __global__ void pack_kernel_u(cuda::kernel_argument<kernel_args<T, Array, Field>, N> args) {
            ...
            if (thread_index < size) {
                ...
                structured::detail::compute_coordinate<Array::size()>::template apply<layout_t>(
                    arg.strides, local_coordinate, thread_index);
                // add offset
        ...
        struct packer<gpu> {
            ...
            static void unpack(BufferMem& m) {
                ...
                await_futures(m.m_recv_futures, [&stream_ptrs](typename BufferMem::hook_type hook) {
                        ...
                        if (p1.second.size > 0u) {
                            ...
                            for (const auto& fb : p1.second.field_infos) {
                                ...
                                for (const auto& it_space_pair : *fb.index_container) {
                                    ...
                                    structured::detail::compute_strides<dimension::value>::template apply<
                                        typename FieldType::layout_map>(local_extents, local_strides);
            ...
            static void unpack_u(BufferMem& m) {
                ...
                std::vector<cudaStream_t*> stream_ptrs;
                stream_ptrs.reserve(m.m_recv_futures.size());
                await_futures(m.m_recv_futures, [&block_size, &stream_ptrs, &args](typename BufferMem::hook_type hook) {
                    auto stream_ptr = &hook->m_cuda_stream.get();
                    args.resize(0);
                    int num_blocks_y = 0;
                    int max_size = 0;
