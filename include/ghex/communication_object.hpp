    ...
    namespace ghex {
        ...
        class communication_object<Pattern, gridtools::ghex::cpu> {
            ...
            std::size_t iteration_spaces_size(const std::vector<iteration_space_t>& iteration_spaces) {
                std::size_t size{0};
                for (const auto& is : iteration_spaces) { size += is.size(); }
            ...
            std::size_t buffer_size(const std::vector<iteration_space_t>& iteration_spaces,
                ...
                for (const auto& is : iteration_spaces) {
                    gridtools::ghex::detail::for_each(
                        data_descriptors, [&is, &size](const auto& dd) { size += is.size() * dd.data_type_size(); });
                }
            ...
            void pack(const std::size_t halo_index, const extended_domain_id_t& domain,
                ...
                gridtools::ghex::detail::for_each(
                    data_descriptors, [this, &iteration_spaces, &halo_index, &buffer_index](const auto& dd) {
                        for (const auto& is : iteration_spaces) {
        ...
        public:
            ...
            class handle {
                ...
                void unpack(const std::size_t halo_index, const extended_domain_id_t& domain) {
                     ...
                     * in order to have as many data of the same type as possible in contiguos memory */
                    gridtools::ghex::detail::for_each(
                        m_data_descriptors, [this, &halo_index, &iteration_spaces, &buffer_index](auto& dd) {
                            for (const auto& is : iteration_spaces) {
            ...
            handle<DataDescriptor...> exchange(DataDescriptor&... dds) {
                ...
                for (const auto& halo : m_ordered_receive_halos) {
                    ...
                    ordered_receive_requests.push_back(
                        std::make_tuple(halo_index, ordered_domain_id,
