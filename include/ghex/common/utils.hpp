...
namespace gridtools {
    ...
    namespace ghex {
        ...
        namespace detail {
            ...
            constexpr int ct_pow(int base, int exp) { return exp == 0 ? 1 : base * ct_pow(base, exp - 1); }
            ...
            void for_each(Tuple&& t, Func&& f) {
                ...
                for_each_impl(std::forward<Tuple>(t), std::forward<Func>(f), std::make_index_sequence<size::value>());
            }
            ...
            void for_each_impl(Tuple1&& t1, Tuple2&& t2, Func&& f, std::index_sequence<Is...>) {
                using std::get;
                invoke_with_2_args(
                    std::forward<Func>(f), //get<Is>(std::forward<Tuple1>(t1), get<Is>(std::forward<Tuple2>(t2))...);
            ...
            public: // static member functions
                ...
                GT_FORCE_INLINE static void apply(Func&& f, Array&& first, Array&& last) noexcept {
                    for (auto i = first[idx::value]; i <= last[idx::value]; ++i) {
                        ...
                        x[idx::value] = i;
                        for_loop<D, I - 1, layout_t>::apply(std::forward<Func>(f), std::forward<Array>(first),
                                                            std::forward<Array>(last), x);
                    }
                }
                ...
                GT_FORCE_INLINE static void apply(Func&& f, Array&& first, Array&& last, Strides&& byte_strides,
                    ...
                    for (auto i = first[idx::value]; i <= last[idx::value]; ++i, ++iter) {
                        for_loop_pointer_arithmetic<D, I - 1, layout_t>::apply(
                            std::forward<Func>(f), std::forward<Array>(first), std::forward<Array>(last),
