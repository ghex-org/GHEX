    ...
    namespace ghex {
        ...
        public: // time functions
            ...
            inline double toc() noexcept {
                const auto t =
                    std::chrono::duration_cast<std::chrono::microseconds>(clock_type::now() - m_time_point).count();
                this->operator()(t);
                return t;
            }
        ...
        timer reduce(const timer& t, MPI_Comm comm) { return reduce(static_cast<accumulator>(t), comm); }
