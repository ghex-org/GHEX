...
struct aligned_allocator_adaptor
  ...
  public: // ctors
    ...
    template<typename Alloc = Allocator, typename std::enable_if<std::is_default_constructible<Alloc>::value,
        int>::type = 0>
    ...
    template<typename U>
    aligned_allocator_adaptor(const aligned_allocator_adaptor<typename base_traits::template rebind_alloc<U>,
        Alignment>& alloc)
        : base_byte{static_cast<typename std::remove_reference_t<decltype(alloc)>::base_byte>(alloc)} {}

    template<typename U>
    aligned_allocator_adaptor(aligned_allocator_adaptor<typename base_traits::template rebind_alloc<U>,
        Alignment>&& alloc)
        : base_byte{static_cast<typename std::remove_reference_t<decltype(alloc)>::base_byte>(alloc)} {}
