...
public: // member functions
    ...
    domain_id_type domain_id() const { return m_id.id; }
};
...
struct make_pattern_impl<detail::structured_grid<CoordinateArrayType>>
{
    ...
    static 
    //std::vector<pattern<P,detail::structured_grid<CoordinateArrayTyp>, typename std::remove_reference_t<DomainRange>::value_type::domain_id_type>>
    auto apply(protocol::setup_communicator& comm, protocol::communicator<P>& new_comm, HaloGenerator&& hgen, DomainRange&& d_range)
    {

//template<typename P, typename CoordinateArrayType, typename HaloGenerator, typename DomainRange>
//auto make_pattern_impl(protocol::setup_communicator& comm, protocol::communicator<P>& new_comm, HaloGenerator&& hgen, DomainRange&& d_range, detail::structured_grid<CoordinateArrayType>*)
//{
    ...
    using pattern_type              = pattern<P, detail::structured_grid<CoordinateArrayType>, domain_id_type>;
    ...
    //protocol::communicator<P> new_comm{comm};
    ...
    const auto num_domain_ids_accumulated = std::accumulate(num_domain_ids.begin(), num_domain_ids.end(), 0);

    std::cout << "rank " << comm.rank() << " (address = " << new_comm.address() << ")\n"
    << "  " << num_domain_ids_accumulated << " (" << my_num_domains << ")\n";
    ...
    // set tags
    // if recv from same processor but different domain
    ...
    for (int rank = 0; rank<(int)num_domain_ids.size(); ++rank)
    {
        if (rank == comm.rank())
        {
