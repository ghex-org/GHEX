...
auto make_pattern_impl(protocol::communicator<protocol::mpi>& comm, HaloGenerator&& hgen, DomainRange&& d_range, _structured_grid<CoordinateArrayType>*)
{
...
template<typename P, typename GridType, typename HaloGenerator, typename DomainRange>
auto make_pattern(protocol::communicator<protocol::mpi>& comm, HaloGenerator&& hgen, DomainRange&& d_range)
{
    using grid_type = typename GridType::template type<typename std::remove_reference_t<DomainRange>::value_type>;
    return detail::make_pattern_impl<P>(comm, std::forward<HaloGenerator>(hgen), std::forward<DomainRange>(d_range), (grid_type*)0); 
}
