...
#include <ghex/common/to_address.hpp>
#include <type_traits>
#include <memory>
#include <cstdint>
#include <cstddef>
...
: public std::allocator_traits<Allocator>::template rebind_alloc<unsigned char>
{
  public: // static constants
    ...
    static constexpr std::uintptr_t offset =
  ...
  public: // member types
    ...
    using propagate_on_container_copy_assignment =
        typename base_byte_traits::propagate_on_container_copy_assignment;
    using propagate_on_container_move_assignment =
        typename base_byte_traits::propagate_on_container_move_assignment;
    ...
    {
        using other =
            aligned_allocator_adaptor<typename base_traits::template rebind_alloc<U>, Alignment>;
    };
  ...
  public: // ctors
    ...
    aligned_allocator_adaptor(const Allocator& alloc)
    : base_byte{alloc}
    {
    }
    aligned_allocator_adaptor(const aligned_allocator_adaptor&) = default;
    aligned_allocator_adaptor(Allocator&& alloc)
    : base_byte{std::move(alloc)}
    {
    }
    aligned_allocator_adaptor(aligned_allocator_adaptor&&) = default;

    template<typename U>
    aligned_allocator_adaptor(const typename base_traits::template rebind_alloc<U>& alloc)
    : base_byte{alloc}
    {
    }
    template<typename U>
    aligned_allocator_adaptor(typename base_traits::template rebind_alloc<U>&& alloc)
    : base_byte{std::move(alloc)}
    {
    }
    ...
    template<typename U>
    aligned_allocator_adaptor(
        const aligned_allocator_adaptor<typename base_traits::template rebind_alloc<U>, Alignment>&
            alloc)
  ...
  public: // member functions
    inline pointer allocate(size_type n)
    {
        ...
        void* vvptr = to_address(vptr);
        void* res =
            reinterpret_cast<void*>((reinterpret_cast<std::uintptr_t>(vvptr) + offset) & mask);
        *((void**)res - 1) = vvptr;
    ...
    inline size_type max_size() const noexcept { return base_byte_traits::max_size(*this); }
    ...
    bool operator!=(const aligned_allocator_adaptor& other) const { return !(operator==(other)); }
};

// fallback to normal Allocator if Alignment is small enough
template<typename Allocator, std::size_t Alignment>
struct aligned_allocator_adaptor<Allocator, Alignment,
    typename std::enable_if<(Alignment <= alignof(std::max_align_t))>::type> : public Allocator
{
