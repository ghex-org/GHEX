...
class communication_object<Pattern, cpu>
{
    ...
    using buffer_t = oomph::message_buffer<byte_t>;
    /** @brief send request type*/
    using s_request_t = oomph::send_request;
    ///** @brief receive request type, 1:1 mapping between receive halo index, domain and receive request*/
    using r_request_t = std::tuple<std::size_t, extended_domain_id_t, oomph::recv_request>;
    ...
    using ordered_r_request_t = std::tuple<std::size_t, ordered_domain_id_t, oomph::recv_request>;
    ...
    std::vector<buffer_t> m_send_buffers;
    std::vector<buffer_t> m_receive_buffers;
        ...
        const std::tuple<DataDescriptor...>&            data_descriptors)
    {
        ...
        for_each(data_descriptors, [ic_num_elements, &size](const auto& dd) {
            size += ic_num_elements *
                    sizeof(typename std::remove_reference_t<decltype(dd)>::value_type);
        });
    ...
    template<typename... DataDescriptor>
    void pack(const std::size_t halo_index, const extended_domain_id_t& domain,
        const std::tuple<DataDescriptor...>& data_descriptors)
    {
        ...
        for_each(data_descriptors, [this, &iteration_spaces, &halo_index, &buffer_index](
                                       const auto& dd) {
            for (const auto& is : iteration_spaces)
            {
                dd.get(is, m_send_buffers[halo_index].data() + buffer_index);
                buffer_index += Pattern::num_elements({is}) *
                                sizeof(typename std::remove_reference_t<decltype(dd)>::value_type);
            }
        });
    }
  ...
  public:
    ...
    class handle
    {
        ...
        void unpack(const std::size_t halo_index, const extended_domain_id_t& domain)
        {
            ...
            /* The two loops are performed with this order
                     * in order to have as many data of the same type as possible in contiguos memory */
            for_each(m_data_descriptors,
                [this, &halo_index, &iteration_spaces, &buffer_index](auto& dd) {
                    for (const auto& is : iteration_spaces)
                    {
                        dd.set(is, m_receive_buffers[halo_index].data() + buffer_index);
                        buffer_index +=
                            Pattern::num_elements({is}) *
                            sizeof(typename std::remove_reference_t<decltype(dd)>::value_type);
                    }
                });
        }
    ...
    /** @brief communication object constructor
             * @param p pattern*/
    communication_object(const Pattern& p, context& c)
    : m_pattern{p}
    : m_ctxt{c.m_ctxt.get()}
      , m_communicator{m_ctxt->get_communicator()}
      ...
      , m_receive_buffers(m_n_receive_halos)
    {
        for (const auto& halo : m_send_halos)
        {
            ...
            ordered_domain_id_t ordered_domain_id{
                domain_id, static_cast<std::size_t>(Pattern::num_elements(iteration_spaces))};
    ...
    //
    //        return {m_receive_halos, m_receive_buffers, std::move(ordered_receive_requests),
    //            std::move(data_descriptors)};
    //    }
};
