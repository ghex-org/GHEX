...
main(int argc, char* argv[])
{
...

    {
        ...
        for (int j = 0; j < inflight; j++)
        {
            ...
            for (int i = 0; i < buff_size; i++) { buffers[j][i] = i % (rank + 1); }
        }
        ...
        while (i < niter)
        {
            ...
            if (flag)
            {
                if (rank == 0)
                {
                    ...
                    MPI_Isend(buffers[j], buff_size, MPI_BYTE, peer_rank, thrid * inflight + j,
                        mpi_comm, &req[j]);
                }
                else
                    MPI_Irecv(buffers[j], buff_size, MPI_BYTE, peer_rank, thrid * inflight + j,
                        mpi_comm, &req[j]);
