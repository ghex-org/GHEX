...
void
send_callback(MsgType mesg, int rank, int tag)
{
...
void
recv_callback(MsgType mesg, int rank, int tag)
{
...
#pragma omp master
        {
            ...
            peer_rank = (rank + 1) % 2;
            if (rank == 0)
                std::cout << "\n\nrunning test " << __FILE__ << " with communicator "
                          << typeid(*comm).name() << "\n\n";
        }
...
#pragma omp barrier
        ...
        for (int j = 0; j < inflight; j++) { available[thrid][j] = 1; }
...
#pragma omp barrier
            ...
            /* complete all comm */
            while (ongoing_comm > 0) { comm->progress(); }
        }
...
#pragma omp critical
        std::cout << "rank " << rank << " thread " << thrid << " submitted " << submit_cnt / nthr
                  << " serviced " << comm_cnt << ", non-local " << nlcomm_cnt
                  << " completion events\n";
