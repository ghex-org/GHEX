...
#endif
        {
            ...
            // https://github.com/openucx/ucx/issues/1162
            {
                ...
                do {
                    ...
                    for (int j = 0; j < inflight; j++) {
                        if (rreqs[j].test()) { rreqs[j] = comm.recv(rmsgs[j], peer_rank, thread_id * inflight + j); }
                    }
                ...
                context.thread_primitives().master(token, [&comm, &sf, &rf, &smsg, &rmsg, peer_rank]() {
                ...
                while (tail_recv == 0) {
                    ...
                    context.thread_primitives().master(token, [&rf]() {
