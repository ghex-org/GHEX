...
struct pool_allocator
{
    ...
    pool_allocator()
    {
        ...
        THREAD_MASTER()
        {
            ...
            if (buffers<T>.size() != nthr) { buffers<T>.resize(nthr); }
        }
        THREAD_BARRIER();
    }
    ...
    {
        if (0 == buffers<T>[thrid].size()) { return m_ba.allocate(size); }
        else
        {
    ...
    void deallocate(T* p, std::size_t size) { buffers<T>[thrid].emplace_back(p, size); }
