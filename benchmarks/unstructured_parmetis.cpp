...
using domain_descriptor_type =
    gridtools::ghex::unstructured::domain_descriptor<domain_id_type, global_index_type>;
using halo_generator_type =
    gridtools::ghex::unstructured::halo_generator<domain_id_type, global_index_type>;
using grid_type = gridtools::ghex::unstructured::grid;
template<typename T>
using data_descriptor_cpu_type =
    gridtools::ghex::unstructured::data_descriptor<gridtools::ghex::cpu, domain_id_type,
        global_index_type, T>;
...
using data_descriptor_gpu_type =
    gridtools::ghex::unstructured::data_descriptor<gridtools::ghex::gpu, domain_id_type,
        global_index_type, T>;
...
std::vector<int>
counts_to_displs(const std::vector<int>& counts)
{
...
void
initialize_field(const Domain& d, Field& f, const O d_id_offset)
{
    using value_type = typename Field::value_type;
    assert(f.size() == d.size() * d.levels());
    for (std::size_t i = 0; i < d.inner_size(); ++i)
    {
        value_type val = static_cast<value_type>(d.domain_id()) * d_id_offset +
                         static_cast<value_type>(d.vertices()[i]);
...
void
check_exchanged_data(const Domain& d, const Pattern& p, const Field& f, const O d_id_offset)
{
    ...
    {
        value_type expected = static_cast<value_type>(pair.second) * d_id_offset +
                              static_cast<value_type>(d.vertices()[pair.first]);
        for (std::size_t level = 0; level < d.levels(); ++level)
        {
...
Domain
make_reindexed_domain(const Domain& d, const Pattern& p)
{
    ...
    for (const auto& rh : p.recv_halos())
    {
        for (const auto i : rh.second.front().local_indices()) { vs.push_back(d.vertices()[i]); }
    }
...
int
domain_to_rank(const DomainId d_id, const int num_threads)
{
...
template<typename DomainId>
std::vector<DomainId>
rank_to_domains(const int rank, const int num_threads)
{
    std::vector<DomainId> res(num_threads);
    for (int i = 0; i < num_threads; ++i) { res[i] = rank * num_threads + i; }
    return res;
}
...
using vertices_dist_type =
    std::map<int, std::map<d_v_pair<domain_id_type, idx_t>, std::vector<idx_t>>>;
using domain_vertices_dist_type = std::map<domain_id_type, std::map<idx_t, std::vector<idx_t>>>;
domain_vertices_dist_type
distribute_parmetis(vertices_dist_type& vertices_dist, std::size_t n_vertices, MPI_Comm comm)
{
    int size;
    ...
    {
        s_n_vertices_rank[i] =
            vertices_dist[i].size(); // any missing rank gets actually inserted into the map here
    };
    std::vector<int> r_n_vertices_rank(size);
    MPI_Alltoall(s_n_vertices_rank.data(), sizeof(int), MPI_BYTE, r_n_vertices_rank.data(),
    ...
    std::vector<idx_t> r_v_ids_rank(
        std::accumulate(r_n_vertices_rank.begin(), r_n_vertices_rank.end(), 0));
    ...
    MPI_Alltoallv(s_v_ids_rank.data(), s_v_ids_rank_counts.data(), s_v_ids_rank_displs.data(),
        MPI_BYTE, r_v_ids_rank.data(), r_v_ids_rank_counts.data(), r_v_ids_rank_displs.data(),
        MPI_BYTE, comm);
    ...
    std::vector<domain_id_type> r_d_ids_rank(
        std::accumulate(r_n_vertices_rank.begin(), r_n_vertices_rank.end(), 0));
    ...
    MPI_Alltoallv(s_d_ids_rank.data(), s_d_ids_rank_counts.data(), s_d_ids_rank_displs.data(),
        MPI_BYTE, r_d_ids_rank.data(), r_d_ids_rank_counts.data(), r_d_ids_rank_displs.data(),
        MPI_BYTE, comm);
    ...
    std::vector<int> s_adjncy_size_vertex_rank_displs =
        counts_to_displs(s_adjncy_size_vertex_rank_counts);
    std::vector<int> r_adjncy_size_vertex_rank(
        std::accumulate(r_n_vertices_rank.begin(), r_n_vertices_rank.end(), 0));
    std::vector<int> r_adjncy_size_vertex_rank_counts = counts_as_bytes<int>(r_n_vertices_rank);
    std::vector<int> r_adjncy_size_vertex_rank_displs =
        counts_to_displs(r_adjncy_size_vertex_rank_counts);
    MPI_Alltoallv(s_adjncy_size_vertex_rank.data(), s_adjncy_size_vertex_rank_counts.data(),
        s_adjncy_size_vertex_rank_displs.data(), MPI_BYTE, r_adjncy_size_vertex_rank.data(),
        r_adjncy_size_vertex_rank_counts.data(), r_adjncy_size_vertex_rank_displs.data(), MPI_BYTE,
        comm);

    // 5) all-to-all: adjacency per rank
    std::vector<idx_t> s_adjncy_rank{};
    s_adjncy_rank.reserve(
        std::accumulate(s_adjncy_size_vertex_rank.begin(), s_adjncy_size_vertex_rank.end(), 0));
    for (const auto& r_m_pair : vertices_dist)
    {
        for (const auto& v_a_pair : r_m_pair.second)
        {
            s_adjncy_rank.insert(s_adjncy_rank.end(), v_a_pair.second.begin(),
                v_a_pair.second.end());
        }
    }
    std::vector<int> s_adjncy_rank_counts{};
    s_adjncy_rank_counts.reserve(size);
    for (auto a_it = s_adjncy_size_vertex_rank.begin(), r_it = s_n_vertices_rank.begin();
         r_it < s_n_vertices_rank.end(); ++r_it)
    {
    ...
    std::vector<idx_t> r_adjncy_rank(
        std::accumulate(r_adjncy_size_vertex_rank.begin(), r_adjncy_size_vertex_rank.end(), 0));
    std::vector<int> r_adjncy_rank_counts{};
    r_adjncy_rank_counts.reserve(size);
    for (auto a_it = r_adjncy_size_vertex_rank.begin(), r_it = r_n_vertices_rank.begin();
         r_it < r_n_vertices_rank.end(); ++r_it)
    {
    ...
    MPI_Alltoallv(s_adjncy_rank.data(), s_adjncy_rank_counts.data(), s_adjncy_rank_displs.data(),
        MPI_BYTE, r_adjncy_rank.data(), r_adjncy_rank_counts.data(), r_adjncy_rank_displs.data(),
        MPI_BYTE, comm);
    ...
    for (std::size_t i = 0, a_idx = 0; i < r_v_ids_rank.size(); ++i)
    {
        ...
        domain_vertices_dist[r_d_ids_rank[i]].insert(
            std::make_pair(r_v_ids_rank[i], std::vector<idx_t>{a_begin, a_end}));
...
TEST(unstructured_parmetis, receive_type)
{
    ...
    static_assert(std::is_same<data_int_type, idx_t>::value,
        "data integer type must be the same as ParMETIS integer type");
    ...
    ap_fs.seekg(
        all_num_vertices / size * sizeof(idx_t) *
        rank); // rewind to begin of section, according to rank (remainder is handled entirely by last rank, TO DO: not optimal)
    std::vector<idx_t> ap{};
    if (rank == (size - 1))
    { // last rank reads until eof
        for (idx_t b; ap_fs.read(as_bytes(b), sizeof(b));) { ap.push_back(b); }
    }
    else
    { // all other ranks read until end of their section
        idx_t section_size =
            all_num_vertices / size +
            1; // (CSR format provides always the two endpoints, first included, second excluded)
    ...
    std::transform(ap.begin(), ap.end(), ap_n.begin(),
        [ap_offset](auto i) { return i - ap_offset; }); // normalize
    ...
    for (int i = 0; i < size; ++i) { vtxdist_v[i] = num_vertices * i; }
    ...
    std::vector<real_t>  tpwgts_v(ncon * nparts,
         1 / static_cast<real_t>(nparts));    // TO DO: might vary
    std::vector<real_t>  ubvec_v(ncon, 1.02); // TO DO: might vary
    std::array<idx_t, 3> options{0, 0, 0};
    idx_t                edgecut;

    // ParMETIS graph partitioning
    ParMETIS_V3_PartKway(vtxdist_v.data(), ap_n.data(), ai.data(), NULL, NULL, &wgtflag, &numflag,
        &ncon, &nparts, tpwgts_v.data(), ubvec_v.data(), options.data(), &edgecut, part_v.data(),
        &comm);
    ...
    {
        vertices_dist[domain_to_rank(part_v[i], num_threads)].insert(std::make_pair(
            d_v_pair<domain_id_type, idx_t>{static_cast<domain_id_type>(part_v[i]), v_id},
            std::vector<idx_t>{ai.begin() + ap_n[i], ai.begin() + ap_n[i + 1]}));
    }
    ...
    for (auto d_id : rank_to_domains<domain_id_type>(gh_rank, num_threads))
    {
        ...
        vertices.reserve(
            domain_vertices_dist[d_id]
                .size()); // any missing domain gets actually inserted into the map here
        std::vector<idx_t>
            adjncy{}; // size may be computed in advance, not preformance critical anyway
        ...
        local_domains.push_back(
            domain_descriptor_type{d_id, vertices, adjncy, levels}); // CSR constructor
    }
    ...
    auto thread_func = [&context, &gh_barrier, &t_buf_local, &t_buf_local_mutex](auto bi)
    {
        ...
        for (int i = 0; i < n_iters_warm_up; ++i)
        { // warm-up
    ...
    for (auto& d : data) { threads.push_back(std::thread{thread_func, p(d)}); }
    ...
    for (const auto& d : local_domains) { n_halo_vertices_local += (d.size() - d.inner_size()); }
    MPI_Allreduce(&n_halo_vertices_local, &n_halo_vertices_global, 1, MPI_INT64_T, MPI_SUM,
        context.mpi_comm()); // MPI type set according to parmetis idx type

    // output
    file << "total exchanged size in GB (assuming value type = idx_t): "
         << static_cast<double>(n_halo_vertices_global * levels * sizeof(idx_t) * 2) /
    ...
    auto p_ord = gridtools::ghex::make_pattern<grid_type>(context, hg,
        local_domains_ord); // easiest way, but quite redundant: only recv halos are different
    ...
    // thread function
    auto thread_func_ord = [&context, &gh_barrier, &t_ord_buf_local, &t_ord_buf_local_mutex](
                               auto bi)
    {
        auto       th_comm = context.get_communicator();
    ...
    file << "2 - ordered halos - buffered receive - CPU\n"
         ...
         << t_ord_buf_global.stddev() / (std::sqrt(t_ord_buf_global.num_samples()) * 1000.0)
         << "ms\n";
    ...
    // thread function
    auto thread_func_ipr = [&context, &gh_barrier, &t_ord_ipr_local, &t_ord_ipr_local_mutex](
                               auto bi)
    {
        auto       th_comm = context.get_communicator();
        timer_type t_ord_ipr_local_th;
        auto       co_ipr =
            gridtools::ghex::make_communication_object_ipr<pattern_container_type>(th_comm);
        for (int i = 0; i < n_iters_warm_up; ++i)
        { // warm-up
    ...
    file << "3 - ordered halos - in-place receive - CPU\n"
         ...
         << t_ord_ipr_global.stddev() / (std::sqrt(t_ord_ipr_global.num_samples()) * 1000.0)
         << "ms\n";
    ...
    std::vector<idx_t> vertices{};
    vertices.reserve(domain_vertices_dist[d_id]
                         .size()); // any missing domain gets actually inserted into the map here
    ...
    GHEX_CHECK_CUDA_RESULT(cudaMemcpy(f_gpu, f_cpu.data(), d.size() * d.levels() * sizeof(idx_t),
    ...
    auto p_ord = gridtools::ghex::make_pattern<grid_type>(context, hg,
        local_domains_ord); // easiest way, but quite redundant: only recv halos are different
    auto co_ord = gridtools::ghex::make_communication_object<pattern_container_type>(
        gh_comm); // new one, same conditions
    ...
    GHEX_CHECK_CUDA_RESULT(cudaMemcpy(f_ord_gpu, f_ord_cpu.data(),
        d_ord.size() * d_ord.levels() * sizeof(idx_t), cudaMemcpyHostToDevice));
    ...
    // check
    cudaMemcpy(f_ord_cpu.data(), f_ord_gpu, d_ord.size() * d_ord.levels() * sizeof(idx_t),
    ...
    GHEX_CHECK_CUDA_RESULT(cudaMemcpy(f_ipr_gpu, f_ipr_cpu.data(),
        d_ord.size() * d_ord.levels() * sizeof(idx_t), cudaMemcpyHostToDevice));
    ...
    // check
    cudaMemcpy(f_ipr_cpu.data(), f_ipr_gpu, d_ord.size() * d_ord.levels() * sizeof(idx_t),
        cudaMemcpyDeviceToHost);
    ...
    idx_t n_halo_vertices_local{static_cast<idx_t>(d.size() - d.inner_size())},
        n_halo_vertices_global;
    MPI_Allreduce(&n_halo_vertices_local, &n_halo_vertices_global, 1, MPI_INT64_T, MPI_SUM,
        context.mpi_comm()); // MPI type set according to parmetis idx type
    file << "total exchanged size in GB (assuming value type = idx_t): "
         << static_cast<double>(n_halo_vertices_global * levels * sizeof(idx_t) * 2) /
    ...
    file << "1 - unordered halos - buffered receive - GPU\n"
         ...
         << t_buf_global_gpu.stddev() / (std::sqrt(t_buf_global_gpu.num_samples()) * 1000.0)
         << "ms\n";

    file << "2 - ordered halos - buffered receive - GPU\n"
         << "\tlocal time = " << t_ord_buf_local_gpu.mean() / 1000.0 << "+/-"
         << t_ord_buf_local_gpu.stddev() / (std::sqrt(t_ord_buf_local_gpu.num_samples()) * 1000.0)
         << "ms\n"
         << "\tglobal time = " << t_ord_buf_global_gpu.mean() / 1000.0 << "+/-"
         << t_ord_buf_global_gpu.stddev() / (std::sqrt(t_ord_buf_global_gpu.num_samples()) * 1000.0)
         << "ms\n";

    file << "3 - ordered halos - in-place receive - GPU\n"
         << "\tlocal time = " << t_ord_ipr_local_gpu.mean() / 1000.0 << "+/-"
         << t_ord_ipr_local_gpu.stddev() / (std::sqrt(t_ord_ipr_local_gpu.num_samples()) * 1000.0)
         << "ms\n"
         << "\tglobal time = " << t_ord_ipr_global_gpu.mean() / 1000.0 << "+/-"
         << t_ord_ipr_global_gpu.stddev() / (std::sqrt(t_ord_ipr_global_gpu.num_samples()) * 1000.0)
         << "ms\n";
