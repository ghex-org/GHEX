...
struct simulation
{
    ...
    using domain_descriptor_type = gridtools::ghex::structured::regular::domain_descriptor<int,
    ...
    using field_descriptor_type = gridtools::ghex::structured::regular::field_descriptor<T, Arch,
        domain_descriptor_type, Layout>;

    using field_type =
        field_descriptor_type<gridtools::ghex::cpu, ::gridtools::layout_map<2, 1, 0>>;
#ifdef GHEX_CUDACC
    using gpu_field_type =
        field_descriptor_type<gridtools::ghex::gpu, ::gridtools::layout_map<2, 1, 0>>;
#endif
    ...
    using pattern_type = std::remove_reference_t<decltype(gridtools::ghex::make_pattern<
        gridtools::ghex::structured::grid>(context, halo_gen, local_domains))>;
    std::unique_ptr<pattern_type> pattern;
    std::mutex                    io_mutex;

    std::vector<gridtools::ghex::timer> timer_vec;

    simulation(int num_reps_, int ext_, int halo, int num_fields_, const decomp_type& decomp_)
    : num_reps{num_reps_}
    , decomp(decomp_)
    ...
    , g_first{0, 0, 0}
    , g_last{decomp.last_coord()[0] * local_ext[0] + local_ext[0] - 1,
    ...
    , local_ext_buffer{local_ext[0] + halos[0] + halos[1], local_ext[1] + halos[2] + halos[3],
    ...
    , timer_vec(num_threads)
    {
        ...
        for (int j = 0; j < num_threads; ++j)
        {
            ...
            local_domains.push_back(domain_descriptor_type{context.rank() * num_threads + j,
                std::array<int, 3>{x, y, z},
                std::array<int, 3>{x + local_ext[0] - 1, y + local_ext[1] - 1,
                    z + local_ext[2] - 1}});
        }

        pattern = std::unique_ptr<pattern_type>{
            new pattern_type{gridtools::ghex::make_pattern<gridtools::ghex::structured::grid>(
    ...
    void exchange()
    {
        ...
        else
        {
            ...
            for (int j = 0; j < num_threads; ++j)
            {
                ...
                int rc =
                    pthread_setaffinity_np(threads[j].native_handle(), sizeof(cpu_set_t), &cpuset);
                if (rc != 0)
                {
    ...
    void exchange(int j)
    {
        ...
        for (int i = 0; i < num_fields; ++i)
        {
            ...
            fields[j].push_back(
                gridtools::ghex::wrap_field<gridtools::ghex::cpu, ::gridtools::layout_map<2, 1, 0>>(
                    local_domains[j], fields_raw[j].back().data(), offset, local_ext_buffer));
#ifdef GHEX_CUDACC
            fields_raw_gpu[j].push_back(std::unique_ptr<T, cuda_deleter<T>>{[this]()
                {
                    void* ptr;
                    cudaMalloc(&ptr, max_memory * sizeof(T));
                    return (T*)ptr;
                }()});
            fields_gpu[j].push_back(
                gridtools::ghex::wrap_field<gridtools::ghex::gpu, ::gridtools::layout_map<2, 1, 0>>(
                    local_domains[j], fields_raw_gpu[j].back().get(), offset, local_ext_buffer));
#endif
        }

        auto bco = gridtools::ghex::bulk_communication_object<
            gridtools::ghex::structured::rma_range_generator, pattern_type,
#ifndef GHEX_CUDACC
            field_type
#else
            gpu_field_type
            ...
            >(basic_co);
#ifndef GHEX_CUDACC
        for (int i = 0; i < num_fields; ++i) bco.add_field(pattern->operator()(fields[j][i]));
#else
        for (int i = 0; i < num_fields; ++i) bco.add_field(pattern->operator()(fields_gpu[j][i]));
#endif
        cos[j] = std::move(bco);

        // warm up
        for (int t = 0; t < 50; ++t) { cos[j].exchange().wait(); }
        ...
        for (int t = 0; t < num_reps; ++t)
        {
            ...
            cos[j].exchange().wait();
            timer_vec[j].toc();
            std::cout << "mean time:    " << comm.rank() << ":" << j << " " << std::setprecision(12)
                      << timer_vec[j].mean() / 1000000.0 << "\n";
            timer_vec[j].clear();
        }
        ...
        if (comm.rank() == 0 && j == 0)
        {
            ...
            std::cout << "sdev f time:  " << std::setprecision(12) << tt.stddev() / tt.mean()
                      << "\n";
            std::cout << "GB/s mean:    " << std::setprecision(12) << load / (tt.mean() * 1000.0)
                      << std::endl;
            std::cout << "GB/s min:     " << std::setprecision(12) << load / (tt.max() * 1000.0)
                      << std::endl;
            std::cout << "GB/s max:     " << std::setprecision(12) << load / (tt.min() * 1000.0)
                      << std::endl;
            std::cout << "GB/s sdev:    " << std::setprecision(12)
                      << (tt.stddev() / tt.mean()) * (load / (tt.mean() * 1000.0)) << std::endl;
        }
    }
};
...
{
    std::cout << "<mpi-launcher> -np N " << app_name << " "
              << "local-domain-size "
              << "num-repetition "
              << "halo-size "
              << "num-fields "
