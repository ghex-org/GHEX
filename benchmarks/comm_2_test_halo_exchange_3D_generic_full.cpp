...
#define C_ADD 2
    ...
    using field_descriptor_type =
        gridtools::ghex::structured::simple_field_wrapper<T, Arch, domain_descriptor_type, Is...>;
    ...
    bool run(ST& file, context_type& context, Comm comm, int DIM1, int DIM2, int DIM3, int H1m1, int H1p1, int H2m1,
             int H2p1, int H3m1, int H3p1, int H1m2, int H1p2, int H2m2, int H2p2, int H3m2, int H3p2, int H1m3,
             int H1p3, int H2m3, int H2p3, int H3m3, int H3p3, triple_t<USE_DOUBLE, T1>* _a,
             triple_t<USE_DOUBLE, T2>* _b, triple_t<USE_DOUBLE, T3>* _c, bool use_gpu) {
        ...
        // wrap raw fields
        auto a = gridtools::ghex::wrap_field<gridtools::ghex::cpu, I1, I2, I3>(
            local_domain.domain_id(), _a, std::array<int, 3>{H1m1, H2m1, H3m1},
            std::array<int, 3>{(DIM1 + H1m1 + H1p1), (DIM2 + H2m1 + H2p1), (DIM3 + H3m1 + H3p1)});
        auto b = gridtools::ghex::wrap_field<gridtools::ghex::cpu, I1, I2, I3>(
            local_domain.domain_id(), _b, std::array<int, 3>{H1m2, H2m2, H3m2},
            std::array<int, 3>{(DIM1 + H1m2 + H1p2), (DIM2 + H2m2 + H2p2), (DIM3 + H3m2 + H3p2)});
        auto c = gridtools::ghex::wrap_field<gridtools::ghex::cpu, I1, I2, I3>(
            local_domain.domain_id(), _c, std::array<int, 3>{H1m3, H2m3, H3m3},
        ...
        // make patterns
        auto pattern_1 =
            gridtools::ghex::make_pattern<gridtools::ghex::structured::grid>(context, halo_gen_1, local_domains);
#ifndef GHEX_1_PATTERN_BENCHMARK
        auto pattern_2 =
            gridtools::ghex::make_pattern<gridtools::ghex::structured::grid>(context, halo_gen_2, local_domains);
        auto pattern_3 =
            gridtools::ghex::make_pattern<gridtools::ghex::structured::grid>(context, halo_gen_3, local_domains);
#endif
        ...
        for (int ii = 0; ii < DIM1; ++ii)
            for (int jj = 0; jj < DIM2; ++jj)
                for (int kk = 0; kk < DIM3; ++kk)
                    a(ii, jj, kk) =
                        triple_t<USE_DOUBLE, T1>(ii + (DIM1)*coords[0], jj + (DIM2)*coords[1], kk + (DIM3)*coords[2]);
        ...
        if (use_gpu) {
...
#ifdef __CUDACC__
            GT_CUDA_CHECK(cudaMalloc(&gpu_a, (DIM1 + H1m1 + H1p1) * (DIM2 + H2m1 + H2p1) * (DIM3 + H3m1 + H3p1) *
                                                 sizeof(triple_t<USE_DOUBLE, T1>::data_type)));
            GT_CUDA_CHECK(cudaMalloc(&gpu_b, (DIM1 + H1m2 + H1p2) * (DIM2 + H2m2 + H2p2) * (DIM3 + H3m2 + H3p2) *
                                                 sizeof(triple_t<USE_DOUBLE, T2>::data_type)));
            GT_CUDA_CHECK(cudaMalloc(&gpu_c, (DIM1 + H1m3 + H1p3) * (DIM2 + H2m3 + H2p3) * (DIM3 + H3m3 + H3p3) *
            ...
            // wrap raw fields
            auto field1 = gridtools::ghex::wrap_field<arch_type, I1, I2, I3>(
                local_domain.domain_id(), gpu_a, std::array<int, 3>{H1m1, H2m1, H3m1},
                std::array<int, 3>{(DIM1 + H1m1 + H1p1), (DIM2 + H2m1 + H2p1), (DIM3 + H3m1 + H3p1)});
            auto field2 = gridtools::ghex::wrap_field<arch_type, I1, I2, I3>(
                local_domain.domain_id(), gpu_b, std::array<int, 3>{H1m2, H2m2, H3m2},
                std::array<int, 3>{(DIM1 + H1m2 + H1p2), (DIM2 + H2m2 + H2p2), (DIM3 + H3m2 + H3p2)});
            auto field3 = gridtools::ghex::wrap_field<arch_type, I1, I2, I3>(
                local_domain.domain_id(), gpu_c, std::array<int, 3>{H1m3, H2m3, H3m3},
                std::array<int, 3>{(DIM1 + H1m3 + H1p3), (DIM2 + H2m3 + H2p3), (DIM3 + H3m3 + H3p3)});
            ...
            for (int k = 0; k < 25; ++k) {
                ...
                file << "TIME PACK/POST:   " << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                     << t_0.mean() / 1000.0 << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                     << t_0_all.mean() / 1000.0 << " ±" << std::scientific << std::setprecision(4) << std::right
                     << std::setw(11) << t_0_all.stddev() / 1000.0 << std::scientific << std::setprecision(4)
                     << std::right << std::setw(12) << t_0_all.min() / 1000.0 << std::scientific << std::setprecision(4)
                     << std::right << std::setw(12) << t_0_all.max() / 1000.0 << std::endl;
                file << "TIME WAIT/UNPACK: " << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                     << t_1.mean() / 1000.0 << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                     << t_1_all.mean() / 1000.0 << " ±" << std::scientific << std::setprecision(4) << std::right
                     << std::setw(11) << t_1_all.stddev() / 1000.0 << std::scientific << std::setprecision(4)
                     << std::right << std::setw(12) << t_1_all.min() / 1000.0 << std::scientific << std::setprecision(4)
                     << std::right << std::setw(12) << t_1_all.max() / 1000.0 << std::endl;
                file << "TIME ALL:         " << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                     << t.mean() / 1000.0 << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                     << t_all.mean() / 1000.0 << " ±" << std::scientific << std::setprecision(4) << std::right
                     << std::setw(11) << t_all.stddev() / 1000.0 << std::scientific << std::setprecision(4)
                     << std::right << std::setw(12) << t_all.min() / 1000.0 << std::scientific << std::setprecision(4)
                     << std::right << std::setw(12) << t_all.max() / 1000.0 << std::endl;
                file << std::endl;
            }

            file << std::endl << "-----------------" << std::endl;
            file << "TIME PACK/POST:   " << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                 << t_0_local.mean() / 1000.0 << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                 << t_0_global.mean() / 1000.0 << " ±" << std::scientific << std::setprecision(4) << std::right
                 << std::setw(11) << t_0_global.stddev() / 1000.0 << std::scientific << std::setprecision(4)
                 << std::right << std::setw(12) << t_0_global.min() / 1000.0 << std::scientific << std::setprecision(4)
                 << std::right << std::setw(12) << t_0_global.max() / 1000.0 << std::endl;
            file << "TIME WAIT/UNPACK: " << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                 << t_1_local.mean() / 1000.0 << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                 << t_1_global.mean() / 1000.0 << " ±" << std::scientific << std::setprecision(4) << std::right
                 << std::setw(11) << t_1_global.stddev() / 1000.0 << std::scientific << std::setprecision(4)
                 << std::right << std::setw(12) << t_1_global.min() / 1000.0 << std::scientific << std::setprecision(4)
                 << std::right << std::setw(12) << t_1_global.max() / 1000.0 << std::endl;
            file << "TIME ALL:         " << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                 << t_local.mean() / 1000.0 << std::scientific << std::setprecision(4) << std::right << std::setw(12)
                 << t_global.mean() / 1000.0 << " ±" << std::scientific << std::setprecision(4) << std::right
                 << std::setw(11) << t_global.stddev() / 1000.0 << std::scientific << std::setprecision(4) << std::right
                 << std::setw(12) << t_global.min() / 1000.0 << std::scientific << std::setprecision(4) << std::right
    ...
    bool test(bool use_gpu, int DIM1, int DIM2, int DIM3, int H1m1, int H1p1, int H2m1, int H2p1, int H3m1, int H3p1,
              int H1m2, int H1p2, int H2m2, int H2p2, int H3m2, int H3p2, int H1m3, int H1p3, int H2m3, int H2p3,
              int H3m3, int H3p3) {
        ...
        */
        ...
        passed = passed && run<std::ostream, 0, 1, 2, true, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);

        file << "run<std::ostream, 0,1,2, true, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, true, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,1,2, true, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, true, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 0,1,2, true, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, true, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,1,2, false, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, false, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 0,1,2, false, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, false, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 0,1,2, false, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, false, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,1,2, false, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, "
                "_a, "
                "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 1, 2, false, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        file << "---------------------------------------------------\n";
        ...
        file << "run<std::ostream, 0,2,1, true, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, true, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,2,1, true, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, true, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,2,1, true, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, true, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 0,2,1, true, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, true, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,2,1, false, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, false, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 0,2,1, false, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, false, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 0,2,1, false, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, false, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 0,2,1, false, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, "
                "_a, "
                "_b, _c)\n";
        passed = passed && run<std::ostream, 0, 2, 1, false, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        file << "---------------------------------------------------\n";
        ...
        file << "run<std::ostream, 1,0,2, true, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, true, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,0,2, true, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, true, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,0,2, true, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, true, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 1,0,2, true, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, true, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,0,2, false, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, false, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 1,0,2, false, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, false, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 1,0,2, false, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, false, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,0,2, false, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, "
                "_a, "
                "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 0, 2, false, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        file << "---------------------------------------------------\n";
        ...
        file << "run<std::ostream, 1,2,0, true, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, true, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,2,0, true, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, true, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,2,0, true, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, true, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 1,2,0, true, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, true, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,2,0, false, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, false, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 1,2,0, false, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, false, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 1,2,0, false, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, false, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 1,2,0, false, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H31, "
                "_a, "
                "_b, _c)\n";
        passed = passed && run<std::ostream, 1, 2, 0, false, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        file << "---------------------------------------------------\n";
        ...
        file << "run<std::ostream, 2,0,1, true, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, true, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,0,1, true, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, true, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,0,1, true, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, true, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 2,0,1, true, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, true, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,0,1, false, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, false, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 2,0,1, false, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, false, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 2,0,1, false, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, false, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,0,1, false, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, "
                "_a, "
                "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 0, 1, false, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        file << "---------------------------------------------------\n";
        ...
        file << "run<std::ostream, 2,1,0, true, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, true, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,1,0, true, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, true, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,1,0, true, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, true, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 2,1,0, true, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, true, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,1,0, false, true, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
                "_b, "
                "_c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, false, true, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 2,1,0, false, true, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, false, true, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file
            << "run<std::ostream, 2,1,0, false, false, true>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, _a, "
               "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, false, false, true>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        ...
        file << "run<std::ostream, 2,1,0, false, false, false>(file, DIM1, DIM2, DIM3, H1m, H1p, H2m, H2p, H3m, H3p, "
                "_a, "
                "_b, _c)\n";
        passed = passed && run<std::ostream, 2, 1, 0, false, false, false>(
                               file, context, comm, DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2,
                               H2m2, H2p2, H3m2, H3p2, H1m3, H1p3, H2m3, H2p3, H3m3, H3p3, _a, _b, _c, use_gpu);
        file << "---------------------------------------------------\n";
...
int main(int argc, char** argv) {
    ...
    halo_exchange_3D_generic_full::test(DIM1, DIM2, DIM3, H1m1, H1p1, H2m1, H2p1, H3m1, H3p1, H1m2, H1p2, H2m2, H2p2,
