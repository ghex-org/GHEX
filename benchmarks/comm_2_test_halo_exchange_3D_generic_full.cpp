...
void printbuff(std::ostream& file, const gridtools::ghex::structured::simple_field_wrapper<T, Arch, DomainDescriptor,
    Order...>& field){
            ...
            for (int jj = 0; jj < DIM2; ++jj)
                    for (int kk = 0; kk < DIM3; ++kk)
                            b(ii, jj, kk) = triple_t<USE_DOUBLE, T2>(
                                ii + (DIM1)*coords[0] + B_ADD, jj + (DIM2)*coords[1] + B_ADD,
                                kk + (DIM3)*coords[2] + B_ADD);

    for (int ii = 0; ii < DIM1; ++ii)
            for (int jj = 0; jj < DIM2; ++jj)
                    for (int kk = 0; kk < DIM3; ++kk)
                            c(ii, jj, kk) = triple_t<USE_DOUBLE, T3>(
                                ii + (DIM1)*coords[0] + C_ADD, jj + (DIM2)*coords[1] + C_ADD,
                                kk + (DIM3)*coords[2] + C_ADD);
...
#ifndef GHEX_1_PATTERN_BENCHMARK
        passed = halo_exchange_3D_generic_full::test(true, Nx, Ny, Nz, 0, 1, 2, 3, 2, 1, 0, 1, 2, 3, 2, 1, 0, 1, 2, 3,
                0, 1);
#else
        //passed = halo_exchange_3D_generic_full::test(true, Nx, Ny, Nz, 0, 1, 2, 3, 2, 1, 0, 1, 2, 3, 2, 1, 0, 1, 2, 3, 2, 1);
        passed = halo_exchange_3D_generic_full::test(true, Nx, Ny, Nz, 3, 3, 3, 3, 0, 0, 3, 3, 3, 3, 0, 0, 3, 3, 3, 3,
                0, 0);
#endif
    }
    else{
...
#ifndef GHEX_1_PATTERN_BENCHMARK
    passed =
        halo_exchange_3D_generic_full::test(false, Nx, Ny, Nz, 0, 1, 2, 3, 2, 1, 0, 1, 2, 3, 2, 1, 0, 1, 2, 3, 0, 1);
#else
