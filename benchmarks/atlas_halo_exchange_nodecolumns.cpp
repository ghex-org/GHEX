...
TEST(atlas_integration, halo_exchange_nodecolumns)
{
    ...
    using cpu_data_descriptor_t = gridtools::ghex::atlas_data_descriptor<gridtools::ghex::cpu,
        domain_id_t, int, storage_traits_cpu, function_space_t>;
    ...
    auto co =
        gridtools::ghex::make_communication_object<decltype(patterns)>(context.get_communicator());
    ...
    auto GHEX_field_1 = gridtools::ghex::atlas::make_field<int, storage_traits_cpu>(fs_nodes,
        1); // 1 component / scalar field
    auto GHEX_field_2 = gridtools::ghex::atlas::make_field<int, storage_traits_cpu>(fs_nodes,
        1); // 1 component / scalar field
    auto GHEX_field_3 = gridtools::ghex::atlas::make_field<int, storage_traits_cpu>(fs_nodes,
        1); // 1 component / scalar field
    auto GHEX_field_4 = gridtools::ghex::atlas::make_field<int, storage_traits_cpu>(fs_nodes,
        1); // 1 component / scalar field
    {
        ...
        for (auto node = 0; node < fs_nodes.nb_nodes(); ++node)
        {
            for (auto level = 0; level < fs_nodes.levels(); ++level)
            {
                ...
                GHEX_field_2_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
                GHEX_field_3_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
                GHEX_field_4_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
            }
        }
    }
    ...
    cpu_data_descriptor_t data_2{local_domains.front(), GHEX_field_2_target_data,
        GHEX_field_2.components()};
    cpu_data_descriptor_t data_3{local_domains.front(), GHEX_field_3_target_data,
        GHEX_field_3.components()};
    cpu_data_descriptor_t data_4{local_domains.front(), GHEX_field_4_target_data,
        GHEX_field_4.components()};
    ...
    auto h = co.exchange(patterns(data_1), patterns(data_2), patterns(data_3),
        patterns(data_4)); // first iteration
    h.wait();
    for (auto i = 0; i < n_iter; ++i)
    { // benchmark
        timer_type t_local;
        ...
        t_local.tic();
        auto h =
            co.exchange(patterns(data_1), patterns(data_2), patterns(data_3), patterns(data_4));
        h.wait();
        t_local.toc();
    ...
    {
        auto atlas_field_1_data =
            atlas::array::make_view<const int, 2>(atlas_fields["atlas_field_1"]);
        auto atlas_field_2_data =
            atlas::array::make_view<const int, 2>(atlas_fields["atlas_field_2"]);
        auto atlas_field_3_data =
            atlas::array::make_view<const int, 2>(atlas_fields["atlas_field_3"]);
        auto atlas_field_4_data =
            atlas::array::make_view<const int, 2>(atlas_fields["atlas_field_4"]);
        ...
        for (auto node = 0; node < fs_nodes.nb_nodes(); ++node)
        {
            for (auto level = 0; level < fs_nodes.levels(); ++level)
            {
                ...
                EXPECT_TRUE(GHEX_field_2_data(node, level, 0) ==
                            atlas_field_2_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
                EXPECT_TRUE(GHEX_field_3_data(node, level, 0) ==
                            atlas_field_3_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
                EXPECT_TRUE(GHEX_field_4_data(node, level, 0) ==
                            atlas_field_4_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
            }
        }
    }
    ...
    file << "- Atlas CPU benchmark\n"
         << "\tlocal time = " << t_atlas_cpu_local.mean() / 1000.0 << "+/-"
         << t_atlas_cpu_local.stddev() / (sqrt(t_atlas_cpu_local.num_samples()) * 1000.0) << "s\n"
         << "\tglobal time = " << t_atlas_cpu_global.mean() / 1000.0 << "+/-"
         << t_atlas_cpu_global.stddev() / (sqrt(t_atlas_cpu_global.num_samples()) * 1000.0)
         << "s\n";

    file << "- GHEX CPU benchmark\n"
         << "\tlocal time = " << t_ghex_cpu_local.mean() / 1000.0 << "+/-"
         << t_ghex_cpu_local.stddev() / (sqrt(t_ghex_cpu_local.num_samples()) * 1000.0) << "s\n"
         << "\tglobal time = " << t_ghex_cpu_global.mean() / 1000.0 << "+/-"
         << t_ghex_cpu_global.stddev() / (sqrt(t_ghex_cpu_global.num_samples()) * 1000.0) << "s\n";
    ...
    using gpu_data_descriptor_t = gridtools::ghex::atlas_data_descriptor<gridtools::ghex::gpu,
        domain_id_t, int, storage_traits_gpu, function_space_t>;

    // Additional fields for GPU halo exchange
    auto GHEX_field_1_gpu = gridtools::ghex::atlas::make_field<int, storage_traits_gpu>(fs_nodes,
        1); // 1 component / scalar field
    auto GHEX_field_2_gpu = gridtools::ghex::atlas::make_field<int, storage_traits_gpu>(fs_nodes,
        1); // 1 component / scalar field
    auto GHEX_field_3_gpu = gridtools::ghex::atlas::make_field<int, storage_traits_gpu>(fs_nodes,
        1); // 1 component / scalar field
    auto GHEX_field_4_gpu = gridtools::ghex::atlas::make_field<int, storage_traits_gpu>(fs_nodes,
        1); // 1 component / scalar field
    {
        ...
        for (auto node = 0; node < fs_nodes.nb_nodes(); ++node)
        {
            for (auto level = 0; level < fs_nodes.levels(); ++level)
            {
                ...
                GHEX_field_1_gpu_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
                GHEX_field_2_gpu_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
                GHEX_field_3_gpu_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
                GHEX_field_4_gpu_data(node, level, 0) =
                    value; // TO DO: hard-coded 3d view. Should be more flexible
            }
        }
    }
    ...
    gpu_data_descriptor_t data_2_gpu{local_domains.front(), 0, GHEX_field_2_gpu_target_data,
        GHEX_field_2_gpu.components()};
    gpu_data_descriptor_t data_3_gpu{local_domains.front(), 0, GHEX_field_3_gpu_target_data,
        GHEX_field_3_gpu.components()};
    gpu_data_descriptor_t data_4_gpu{local_domains.front(), 0, GHEX_field_4_gpu_target_data,
        GHEX_field_4_gpu.components()};

    // GHEX halo exchange on GPU
    auto h_gpu = co.exchange(patterns(data_1_gpu), patterns(data_2_gpu), patterns(data_3_gpu),
        patterns(data_4_gpu)); // first iteration
    h_gpu.wait();
    for (auto i = 0; i < n_iter; ++i)
    { // benchmark
        timer_type t_local;
        ...
        for (auto node = 0; node < fs_nodes.nb_nodes(); ++node)
        {
            for (auto level = 0; level < fs_nodes.levels(); ++level)
            {
                EXPECT_TRUE(GHEX_field_1_gpu_data(node, level, 0) ==
                            atlas_field_1_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
                EXPECT_TRUE(GHEX_field_2_gpu_data(node, level, 0) ==
                            atlas_field_2_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
                EXPECT_TRUE(GHEX_field_3_gpu_data(node, level, 0) ==
                            atlas_field_3_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
                EXPECT_TRUE(GHEX_field_4_gpu_data(node, level, 0) ==
                            atlas_field_4_data(node,
                                level)); // TO DO: hard-coded 3d view. Should be more flexible
            }
        }
    ...
    file << "- GHEX GPU benchmark\n"
         ...
         << t_ghex_gpu_local.stddev() / (sqrt(t_ghex_gpu_local.num_samples()) * 1000.0) << "s\n"
         << "\tglobal time = " << t_ghex_gpu_global.mean() / 1000.0 << "+/-"
         << t_ghex_gpu_global.stddev() / (sqrt(t_ghex_gpu_global.num_samples()) * 1000.0) << "s\n";
