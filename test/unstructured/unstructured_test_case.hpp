...
void
initialize_data(const domain_descriptor_type& d, Container& field, std::size_t levels = 1u,
    ...
    std::size_t levels = 1u, bool levels_first = true)
{
    ...
    if (levels_first)
        for (auto [idx, did] : halo_map)
            for (std::size_t level = 0u; level < levels; ++level)
                EXPECT_EQ(field[idx * levels + level],
                    static_cast<value_type>(
                        did * 10000 + d.global_index(idx).value() * 100 + level));
    else
        for (std::size_t level = 0u; level < levels; ++level)
            for (auto [idx, did] : halo_map)
                EXPECT_EQ(field[idx + level * d.size()],
                    static_cast<value_type>(
                        did * 10000 + d.global_index(idx).value() * 100 + level));
}
