...
{
    bool operator()(const cubed_sphere_field_descriptor& lhs,
        const cubed_sphere_field_descriptor&             rhs) const
    {
...
using grid_detail_type =
    ghex::structured::detail::grid<ghex::coordinate<std::array<int, 4>>>; // only 3D grids
...
using communication_obj_type =
    ghex::communication_object<communicator_type, grid_detail_type, domain_id_type>;
...
// row-major storage
using field_descriptor_type_1 = ghex::structured::cubed_sphere::field_descriptor<fp_type, arch_type,
    ::gridtools::layout_map<3, 2, 1, 0>>;
using pattern_field_type_1 =
    ghex::buffer_info<pattern_type::value_type, arch_type, field_descriptor_type_1>;
using pattern_field_vector_type_1 = std::pair<std::vector<std::unique_ptr<field_descriptor_type_1>>,
    std::vector<pattern_field_type_1>>;

// field-major storage
using field_descriptor_type_2 = ghex::structured::cubed_sphere::field_descriptor<fp_type, arch_type,
    ::gridtools::layout_map<2, 1, 0, 3>>;
using pattern_field_type_2 =
    ghex::buffer_info<pattern_type::value_type, arch_type, field_descriptor_type_2>;
using pattern_field_vector_type_2 = std::pair<std::vector<std::unique_ptr<field_descriptor_type_2>>,
    std::vector<pattern_field_type_2>>;
...
extern "C" void
ghex_cubed_sphere_co_init(obj_wrapper** wco_ref, obj_wrapper* wcomm)
{
...
extern "C" void
ghex_cubed_sphere_domain_add_field(cubed_sphere_domain_descriptor* domain_desc,
    cubed_sphere_field_descriptor*                                 field_desc)
{
...
extern "C" void
ghex_cubed_sphere_domain_free(cubed_sphere_domain_descriptor* domain_desc)
{
...
extern "C" void*
ghex_cubed_sphere_exchange_desc_new(cubed_sphere_domain_descriptor* domains_desc, int n_domains)
{
    ...
    for (int i = 0; i < n_domains; i++)
    {
        ...
        local_domains.emplace_back(c, domains_desc[i].tile, domains_desc[i].first[0] - 1,
            domains_desc[i].last[0] - 1, domains_desc[i].first[1] - 1, domains_desc[i].last[1] - 1);
    }
    ...
    for (int i = 0; i < n_domains; i++)
    {
        ...
        for (auto field : fields)
        {
            ...
            if (pit == field_to_pattern.end())
            {
                ...
                pit = field_to_pattern
                          .emplace(std::make_pair(std::move(field),
                              ghex::make_pattern<grid_type>(*ghex_context, halo_generator,
                                  local_domains)))
                          .first;
            }
            ...
            {
                std::unique_ptr<field_descriptor_type_1> field_desc_uptr(
                    new field_descriptor_type_1(local_domains[i], field.data, offset, extents,
                        field.n_components, field.is_vector));
            ...
            {
                std::unique_ptr<field_descriptor_type_2> field_desc_uptr(
                    new field_descriptor_type_2(local_domains[i], field.data, offset, extents,
                        field.n_components, field.is_vector));
                auto ptr = field_desc_uptr.get();
...
extern "C" void*
ghex_cubed_sphere_exchange(obj_wrapper* cowrapper, obj_wrapper* ewrapper)
{
    ...
    return new obj_wrapper(
        co.exchange(pattern_fields.row_major.second.begin(), pattern_fields.row_major.second.end(),
            pattern_fields.field_major.second.begin(), pattern_fields.field_major.second.end()));
}

extern "C" void
ghex_cubed_sphere_exchange_handle_wait(obj_wrapper** ehwrapper)
{
