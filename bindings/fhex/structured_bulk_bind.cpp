...
ghex_struct_co_init(obj_wrapper** wco_ref)
{
    ...
    auto bco = ghex::bulk_communication_object<ghex::structured::rma_range_generator, pattern_type,
        field_descriptor_type>(context());
    *wco_ref = new obj_wrapper(bco_wrapper{std::move(bco), nullptr});
}
...
ghex_struct_exchange(obj_wrapper* cowrapper, obj_wrapper* ewrapper)
{
    ...
    return new obj_wrapper(bco.exchange());
}
