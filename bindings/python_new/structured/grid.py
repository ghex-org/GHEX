...
class grid_3d:
    ...
    def __init__(self, Nx, Ny, Nz, *, bcx=1, bcy=1, bcz=1):
        ...
        ijk = self._init_index_space(
            (Nx, Ny, Nz), (bcx, bcy, bcz), (1, 1, 1), (False, False, False)
        )
        ijk_sx = self._init_index_space(
            (Nx + 1, Ny, Nz), (bcx, bcy, bcz), (1, 0, 0), (True, False, False)
        )
        ijk_sy = self._init_index_space(
            (Nx, Ny + 1, Nz), (bcx, bcy, bcz), (0, 1, 0), (False, True, False)
        )
        ijk_sz = self._init_index_space(
    ...
    def _init_index_space(
        ...
        def label_from_dir(dir_):
            ...
            return "-".join(
                labels[l][0 if dir_l == -1 else 1]
                for l, dir_l in enumerate(permuted_dir)
                if dir_l != 0
            )
        ...
        index_space.add_subset(
            ...
            .extend(
                *(
                    el if hb and bc == 1 else 0
                    for hb, bc, el in zip(has_boundary, bcs, periodic_layers)
                )
            )
            .without(index_space.subset["physical"]),
        )
        ...
        index_space.add_subset(
            ("interior", (1, 0, 0)), index_space.covering.simplify().extend(-1, 0, 0)
        )
        index_space.add_subset(
            ("interior", (0, 1, 0)), index_space.covering.simplify().extend(0, -1, 0)
        )
        if shape[2] != 1:  # not meaningful for pseudo horizontal
            index_space.add_subset(
                ("interior", (0, 0, 1)),
                index_space.covering.simplify().extend(0, 0, -1),
            )

        assert (
            union(interior, prescribed_boundary, periodic_boundary)
            == index_space.subset["definition"]
        )
        assert (
            union(index_space.subset["physical"], periodic_boundary)
            == index_space.subset["definition"]
        )
    ...
    def _init_periodic_images(self):
        ...
        for id_, index_space in original_index_spaces.items():
            ...
            for dir in unit_range(-1, 2) * unit_range(-1, 2) * unit_range(-1, 2):
                ...
                self.index_conventions[image_id] = {image_id: (0, 0, 0), id_: (0, 0, 0)}
            ...
            assert (
                ...
                *(
                    ...
                    for image_index_space in self.periodic_images[id_].values()
                )
            ) == index_space.subset[
                "periodic_layers"
            ]
