...
struct array {
    ...
    array(T *_p, int _n, int _m, int _l)
        : ptr(_p), n(gridtools::make_array(_n, _m, _l)[lmap::find(0)]),
          m(gridtools::make_array(_n, _m, _l)[lmap::find(1)]),
          l(gridtools::make_array(_n, _m, _l)[lmap::find(2)]) {}

    T &operator()(int i, int j, int k) {
        // a[(DIM1+2*H)*(DIM2+2*H)*kk+ii*(DIM2+2*H)+jj]
        return ptr[l * m * gridtools::make_array(i, j, k)[lmap::find(0)] +
                   l * gridtools::make_array(i, j, k)[lmap::find(1)] +
                   gridtools::make_array(i, j, k)[lmap::find(2)]];
    }
