...
#ifdef __CUDACC__
#include <stdio.h>
__global__ void print_kernel() { printf("Hello from block %d, thread %d\n", blockIdx.x, threadIdx.x); }
#endif
...
template<typename T, typename Domain, typename Halos, typename Periodic, typename Global, typename Field,
         typename Communicator>
bool test_values(const Domain& d, const Halos& halos, const Periodic& periodic, const Global& g_first,
                 const Global& g_last, const Field& f, Communicator& comm) {
    bool passed = true;
...
TEST(communication_object_2, exchange) {
    ...
    if (local_comm.rank() < num_devices_per_node) {
        std::cout << "I am rank " << mpi_comm.rank() << " and I own GPU "
                  << (context.world().rank() / local_comm.size()) * num_devices_per_node + local_comm.rank()
                  << std::endl;
    ...
    const std::array<int, 3> g_last{local_ext[0] * 4 - 1, ((context.world().size() - 1) / 2 + 1) * local_ext[1] - 1,
                                    local_ext[2] - 1};
    // maximum halo
    const std::array<int, 3> offset{3, 3, 3};
    // local size including potential halos
    const std::array<int, 3> local_ext_buffer{local_ext[0] + 2 * offset[0], local_ext[1] + 2 * offset[1],
    ...
    // allocate fields
#if defined(GHEX_TEST_SERIAL_VECTOR) || defined(GHEX_TEST_SERIAL_SPLIT_VECTOR) || defined(GHEX_TEST_THREADS_VECTOR) || \
    defined(GHEX_TEST_ASYNC_ASYNC_VECTOR) || defined(GHEX_TEST_ASYNC_DEFERRED_VECTOR) ||                               \
    ...
    local_domains.push_back(domain_descriptor_type{
        ...
        std::array<int, 3>{((context.world().rank() % 2) * 2) * local_ext[0],
                           (context.world().rank() / 2) * local_ext[1], 0},
        std::array<int, 3>{((context.world().rank() % 2) * 2 + 1) * local_ext[0] - 1,
                           (context.world().rank() / 2 + 1) * local_ext[1] - 1, local_ext[2] - 1}});
    local_domains.push_back(domain_descriptor_type{
        context.world().rank() * 2 + 1,
        std::array<int, 3>{((context.world().rank() % 2) * 2 + 1) * local_ext[0],
                           (context.world().rank() / 2) * local_ext[1], 0},
        std::array<int, 3>{((context.world().rank() % 2) * 2 + 2) * local_ext[0] - 1,
                           (context.world().rank() / 2 + 1) * local_ext[1] - 1, local_ext[2] - 1}});
    ...
    // wrap raw fields
    auto field_1a = gridtools::ghex::wrap_field<gridtools::ghex::cpu, 2, 1, 0>(
        local_domains[0].domain_id(), field_1a_raw.data(), offset, local_ext_buffer);
    auto field_1b = gridtools::ghex::wrap_field<gridtools::ghex::cpu, 2, 1, 0>(
        local_domains[1].domain_id(), field_1b_raw.data(), offset, local_ext_buffer);
    auto field_2a = gridtools::ghex::wrap_field<gridtools::ghex::cpu, 2, 1, 0>(
        local_domains[0].domain_id(), field_2a_raw.data(), offset, local_ext_buffer);
    auto field_2b = gridtools::ghex::wrap_field<gridtools::ghex::cpu, 2, 1, 0>(
        local_domains[1].domain_id(), field_2b_raw.data(), offset, local_ext_buffer);
    auto field_3a = gridtools::ghex::wrap_field<gridtools::ghex::cpu, 2, 1, 0>(
        local_domains[0].domain_id(), field_3a_raw.data(), offset, local_ext_buffer);
    auto field_3b = gridtools::ghex::wrap_field<gridtools::ghex::cpu, 2, 1, 0>(
...
#error                                                                                                                 \
    "At least one of the following macros should be defined: GHEX_TEST_SERIAL GHEX_TEST_SERIAL_SPLIT GHEX_TEST_EXCHANGE_THREADS GHEX_TEST_ASYNC_ASYNC GHEX_TEST_ASYNC_DEFERRED GHEX_TEST_ASYNC_ASYNC_WAIT"
#endif
#endif
#endif
#endif
...
#error "hybrid tests are not possible with vector interface"
    ...
    if (local_comm.rank() < num_devices_per_node) {
        ...
        // wrap raw fields
        auto field_1a_gpu = gridtools::ghex::wrap_field<gridtools::ghex::gpu, 2, 1, 0>(
            local_domains[0].domain_id(), gpu_1a_raw, offset, local_ext_buffer);
        auto field_2a_gpu = gridtools::ghex::wrap_field<gridtools::ghex::gpu, 2, 1, 0>(
            local_domains[0].domain_id(), gpu_2a_raw, offset, local_ext_buffer);
        auto field_3a_gpu = gridtools::ghex::wrap_field<gridtools::ghex::gpu, 2, 1, 0>(
            local_domains[0].domain_id(), gpu_3a_raw, offset, local_ext_buffer);
#ifndef GHEX_HYBRID_TESTS
        auto field_1b_gpu = gridtools::ghex::wrap_field<gridtools::ghex::gpu, 2, 1, 0>(
            local_domains[1].domain_id(), gpu_1b_raw, offset, local_ext_buffer);
        auto field_2b_gpu = gridtools::ghex::wrap_field<gridtools::ghex::gpu, 2, 1, 0>(
            local_domains[1].domain_id(), gpu_2b_raw, offset, local_ext_buffer);
        auto field_3b_gpu = gridtools::ghex::wrap_field<gridtools::ghex::gpu, 2, 1, 0>(
            local_domains[1].domain_id(), gpu_3b_raw, offset, local_ext_buffer);
#endif

#ifdef __CUDACC__
        // copy
        GT_CUDA_CHECK(
            cudaMemcpy(field_1a_gpu.data(), field_1a.data(), max_memory * sizeof(TT1), cudaMemcpyHostToDevice));
        GT_CUDA_CHECK(
            cudaMemcpy(field_2a_gpu.data(), field_2a.data(), max_memory * sizeof(TT2), cudaMemcpyHostToDevice));
        GT_CUDA_CHECK(
            cudaMemcpy(field_3a_gpu.data(), field_3a.data(), max_memory * sizeof(TT3), cudaMemcpyHostToDevice));
#ifndef GHEX_HYBRID_TESTS
        GT_CUDA_CHECK(
            cudaMemcpy(field_1b_gpu.data(), field_1b.data(), max_memory * sizeof(TT1), cudaMemcpyHostToDevice));
        GT_CUDA_CHECK(
            cudaMemcpy(field_2b_gpu.data(), field_2b.data(), max_memory * sizeof(TT2), cudaMemcpyHostToDevice));
        GT_CUDA_CHECK(
            cudaMemcpy(field_3b_gpu.data(), field_3b.data(), max_memory * sizeof(TT3), cudaMemcpyHostToDevice));
#endif
#else
        ...
        co.bexchange(pattern1(field_1a_gpu), pattern1(field_1b), pattern2(field_2a_gpu), pattern2(field_2b),
                     pattern1(field_3a_gpu), pattern1(field_3b));
#else
        auto co = gridtools::ghex::make_communication_object<decltype(pattern1)>(
            context.get_communicator(context.get_token()));
        co.bexchange(pattern1(field_1a_gpu), pattern1(field_1b_gpu), pattern2(field_2a_gpu), pattern2(field_2b_gpu),
                     pattern1(field_3a_gpu), pattern1(field_3b_gpu));
#endif
#endif
#ifdef GHEX_TEST_SERIAL_VECTOR
        auto co = gridtools::ghex::make_communication_object<decltype(pattern1)>(
        ...
        std::vector<std::remove_reference_t<decltype(pattern1(field_1a_gpu))>> field_vec{
            pattern1(field_1a_gpu), pattern1(field_1b_gpu), pattern2(field_2a_gpu),
            pattern2(field_2b_gpu), pattern1(field_3a_gpu), pattern1(field_3b_gpu)};
        co.exchange(field_vec.data(), field_vec.size()).wait();
#endif

#ifdef GHEX_TEST_SERIAL_SPLIT
        auto token = context.get_token();
        auto co_1 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(token));
        // non-blocking variant
        auto h1 = co_1.exchange(pattern1(field_1a_gpu), pattern2(field_2a_gpu), pattern1(field_3a_gpu));
#ifdef GHEX_HYBRID_TESTS
        auto co_2 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(token));
        ...
        std::vector<std::remove_reference_t<decltype(pattern1(field_1a_gpu))>> field_vec_a{
            pattern1(field_1a_gpu), pattern2(field_2a_gpu), pattern1(field_3a_gpu)};
        std::vector<std::remove_reference_t<decltype(pattern1(field_1a_gpu))>> field_vec_b{
            pattern1(field_1b_gpu), pattern2(field_2b_gpu), pattern1(field_3b_gpu)};
        ...
        auto func = [&context](auto... bis) {
            auto co_ = gridtools::ghex::make_communication_object<decltype(pattern1)>(
                context.get_communicator(context.get_token()));
            co_.bexchange(bis...);
        };
        ...
        threads.push_back(std::thread{func, pattern1(field_1a_gpu), pattern2(field_2a_gpu), pattern1(field_3a_gpu)});
#ifdef GHEX_HYBRID_TESTS
        threads.push_back(std::thread{func, pattern1(field_1b), pattern2(field_2b), pattern1(field_3b)});
#else
        threads.push_back(std::thread{func, pattern1(field_1b_gpu), pattern2(field_2b_gpu), pattern1(field_3b_gpu)});
#endif
        ...
        // waiting and unpacking may be done concurrently
        std::vector<std::thread> threads;
        field_vec_type field_vec_a{pattern1(field_1a_gpu), pattern2(field_2a_gpu), pattern1(field_3a_gpu)};
        field_vec_type field_vec_b{pattern1(field_1b_gpu), pattern2(field_2b_gpu), pattern1(field_3b_gpu)};
        ...
        auto future_1 =
            std::async(policy, func, pattern1(field_1a_gpu), pattern2(field_2a_gpu), pattern1(field_3a_gpu));
#ifdef GHEX_HYBRID_TESTS
        auto future_2 = std::async(policy, func, pattern1(field_1b), pattern2(field_2b), pattern1(field_3b));
#else
        auto future_2 =
            std::async(policy, func, pattern1(field_1b_gpu), pattern2(field_2b_gpu), pattern1(field_3b_gpu));
#endif
        // ... overlap communication with computation here
        future_1.wait();
        future_2.wait();
        ...
        auto func_h = [](auto co_, auto... bis) { return co_->exchange(bis...); };
        auto token = context.get_token();
        auto co_1 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(token));
        auto co_2 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(token));
        ...
        auto future_1 =
            std::async(policy, func_h, &co_1, pattern1(field_1a_gpu), pattern2(field_2a_gpu), pattern1(field_3a_gpu));
#ifdef GHEX_HYBRID_TESTS
        auto future_2 = std::async(policy, func_h, &co_2, pattern1(field_1b), pattern2(field_2b), pattern1(field_3b));
#else
        auto future_2 =
            std::async(policy, func_h, &co_2, pattern1(field_1b_gpu), pattern2(field_2b_gpu), pattern1(field_3b_gpu));
#endif
        ...
        using field_vec_type = std::vector<std::remove_reference_t<decltype(pattern1(field_1a_gpu))>>;
        auto func_h = [](auto co_, field_vec_type& vec) { return co_->exchange(vec.data(), vec.size()); };
        auto token = context.get_token();
        auto co_1 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(token));
        auto co_2 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(token));
        // packing and posting may be done concurrently
    ...
    auto co_1 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(context.get_token()));
    auto co_2 = gridtools::ghex::make_communication_object<decltype(pattern1)>(context.get_communicator(context.get_token()));
    // packing and posting may be done concurrently
    ...
    future_2.get().wait();
        ...
        // copy back
        GT_CUDA_CHECK(
            cudaMemcpy(field_1a.data(), field_1a_gpu.data(), max_memory * sizeof(TT1), cudaMemcpyDeviceToHost));
        GT_CUDA_CHECK(
            cudaMemcpy(field_2a.data(), field_2a_gpu.data(), max_memory * sizeof(TT2), cudaMemcpyDeviceToHost));
        GT_CUDA_CHECK(
            cudaMemcpy(field_3a.data(), field_3a_gpu.data(), max_memory * sizeof(TT3), cudaMemcpyDeviceToHost));
#ifndef GHEX_HYBRID_TESTS
        GT_CUDA_CHECK(
            cudaMemcpy(field_1b.data(), field_1b_gpu.data(), max_memory * sizeof(TT1), cudaMemcpyDeviceToHost));
        GT_CUDA_CHECK(
            cudaMemcpy(field_2b.data(), field_2b_gpu.data(), max_memory * sizeof(TT2), cudaMemcpyDeviceToHost));
        GT_CUDA_CHECK(
            cudaMemcpy(field_3b.data(), field_3b_gpu.data(), max_memory * sizeof(TT3), cudaMemcpyDeviceToHost));
#endif
...
#endif // ifdef CUDA or HYBRID
    {
        ...
        auto co = gridtools::ghex::make_communication_object<decltype(pattern1)>(
            context.get_communicator(context.get_token()));
        co.bexchange(pattern1(field_1a), pattern1(field_1b), pattern2(field_2a), pattern2(field_2b), pattern1(field_3a),
                     pattern1(field_3b));
#endif
#ifdef GHEX_TEST_SERIAL_VECTOR
        auto co = gridtools::ghex::make_communication_object<decltype(pattern1)>(
        ...
        std::vector<std::remove_reference_t<decltype(pattern1(field_1a))>> field_vec{
            pattern1(field_1a), pattern1(field_1b), pattern2(field_2a),
            pattern2(field_2b), pattern1(field_3a), pattern1(field_3b)};
        co.exchange(field_vec.data(), field_vec.size()).wait();
#endif
        ...
        std::vector<std::remove_reference_t<decltype(pattern1(field_1a))>> field_vec_a{
            pattern1(field_1a), pattern2(field_2a), pattern1(field_3a)};
        std::vector<std::remove_reference_t<decltype(pattern1(field_1a))>> field_vec_b{
            pattern1(field_1b), pattern2(field_2b), pattern1(field_3b)};
        auto h1 = co_1.exchange(field_vec_a.data(), field_vec_a.size());
        auto h2 = co_2.exchange(field_vec_b.data(), field_vec_b.size());
        // ... overlap communication (packing, posting) with computation here
        // wait and upack:
        ...
        // waiting and unpacking may be done concurrently
        std::vector<std::thread> threads;
        threads.push_back(std::thread{func, pattern1(field_1a), pattern2(field_2a), pattern1(field_3a)});
        threads.push_back(std::thread{func, pattern1(field_1b), pattern2(field_2b), pattern1(field_3b)});
        // ... overlap communication with computation here
        for (auto& t : threads) t.join();
#endif
        ...
        // waiting and unpacking may be done concurrently
        std::vector<std::thread> threads;
        field_vec_type field_vec_a{pattern1(field_1a), pattern2(field_2a), pattern1(field_3a)};
        field_vec_type field_vec_b{pattern1(field_1b), pattern2(field_2b), pattern1(field_3b)};
        threads.push_back(std::thread{func, std::ref(field_vec_a)});
        threads.push_back(std::thread{func, std::ref(field_vec_b)});
        // ... overlap communication with computation here
        for (auto& t : threads) t.join();
        ...
        // waiting and unpacking may be done concurrently
        auto policy = std::launch::async;
        auto future_1 = std::async(policy, func, pattern1(field_1a), pattern2(field_2a), pattern1(field_3a));
        auto future_2 = std::async(policy, func, pattern1(field_1b), pattern2(field_2b), pattern1(field_3b));
        // ... overlap communication with computation here
        future_1.wait();
        future_2.wait();
        ...
        // waiting and unpacking serially on current thread
        auto policy = std::launch::deferred;
        auto future_1 = std::async(policy, func_h, &co_1, pattern1(field_1a), pattern2(field_2a), pattern1(field_3a));
        auto future_2 = std::async(policy, func_h, &co_2, pattern1(field_1b), pattern2(field_2b), pattern1(field_3b));
        // deferred policy: essentially serial on current thread
        auto h1 = future_1.get();
        auto h2 = future_2.get();
