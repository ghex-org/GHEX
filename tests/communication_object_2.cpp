...
#error "hybrid tests are not possible with vector interface"
...
#ifdef GHEX_HYBRID_TESTS
    co.exchange(
        pattern1.halo(field_1a_gpu),
        pattern1.halo(field_1b),
        pattern2.halo(field_2a_gpu),
        pattern2.halo(field_2b),
        pattern1.halo(field_3a_gpu),
        pattern1.halo(field_3b)
    ).wait();
#else
    co.exchange(
        pattern1.halo(field_1a_gpu),
        pattern1.halo(field_1b_gpu),
        pattern2.halo(field_2a_gpu),
        pattern2.halo(field_2b_gpu),
        pattern1.halo(field_3a_gpu),
        pattern1.halo(field_3b_gpu)
    ).wait();
#endif
#endif
#ifdef GHEX_TEST_SERIAL_VECTOR
    std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a_gpu))>> field_vec{
        pattern1.halo(field_1a_gpu),
        pattern1.halo(field_1b_gpu),
        pattern2.halo(field_2a_gpu),
        pattern2.halo(field_2b_gpu),
    ...
    auto h1 = co_1.exchange(pattern1.halo(field_1a_gpu), pattern2.halo(field_2a_gpu), pattern1.halo(field_3a_gpu));
#ifdef GHEX_HYBRID_TESTS
    auto h2 = co_2.exchange(pattern1.halo(field_1b), pattern2.halo(field_2b), pattern1.halo(field_3b));
#else
    auto h2 = co_2.exchange(pattern1.halo(field_1b_gpu), pattern2.halo(field_2b_gpu), pattern1.halo(field_3b_gpu));
#endif
    ...
    std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a_gpu))>> field_vec_a{
        pattern1.halo(field_1a_gpu),
        pattern2.halo(field_2a_gpu),
        pattern1.halo(field_3a_gpu)};
    std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a_gpu))>> field_vec_b{
        pattern1.halo(field_1b_gpu),
        pattern2.halo(field_2b_gpu),
        pattern1.halo(field_3b_gpu)};
    ...
    using field_vec_type = std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a_gpu))>>;
    ...
    // blocking variant
    co.exchange(
        pattern1.halo(field_1a),
        pattern1.halo(field_1b),
        pattern2.halo(field_2a),
        pattern2.halo(field_2b),
        pattern1.halo(field_3a),
        pattern1.halo(field_3b)
    ).wait();
#endif
#ifdef GHEX_TEST_SERIAL_VECTOR
    std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a))>> field_vec{
        pattern1.halo(field_1a),
        pattern1.halo(field_1b),
        pattern2.halo(field_2a),
        pattern2.halo(field_2b),
...
#ifdef GHEX_TEST_SERIAL_SPLIT
    // non-blocking variant
    auto h1 = co_1.exchange(pattern1.halo(field_1a), pattern2.halo(field_2a), pattern1.halo(field_3a));
    auto h2 = co_2.exchange(pattern1.halo(field_1b), pattern2.halo(field_2b), pattern1.halo(field_3b));
    // ... overlap communication (packing, posting) with computation here
    // wait and upack:
    h1.wait();
...
#endif
#ifdef GHEX_TEST_SERIAL_SPLIT_VECTOR
    std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a))>> field_vec_a{
        pattern1.halo(field_1a),
        pattern2.halo(field_2a),
        pattern1.halo(field_3a)};
    std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a))>> field_vec_b{
        pattern1.halo(field_1b),
        pattern2.halo(field_2b),
        pattern1.halo(field_3b)};
    auto h1 = co_1.exchange(field_vec_a.data(), field_vec_a.size());
...
#endif
#ifdef GHEX_TEST_THREADS_VECTOR
    using field_vec_type = std::vector<std::remove_reference_t<decltype(pattern1.halo(field_1a))>>;
    auto func = [](decltype(co)& co_, field_vec_type& vec)
    {
        co_.exchange(vec.data(), vec.size()).wait();
    };
