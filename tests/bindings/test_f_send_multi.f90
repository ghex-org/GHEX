  ...
  block
    ...
    do while (it<mpi_size)
      if (it/=mpi_rank) then        
        ...
        call ghex_comm_recv_cb(comm, rmsg(it), it, it, pcb, user_data=user_data, req=rrequests(it))
      end if
      it = it+1
    end do
    !$omp end master
    ...
    do while(it < mpi_size)
      if (it /= mpi_rank) then
        do while (rank_received(it) /= nthreads*2)
...
CONTAINS
  ...
  subroutine send_callback (mesg, rank, tag, user_data) bind(c)
    ...
    print *, "send_multi completed on mpi_rank", tag
  ...
  subroutine recv_callback (mesg, rank, tag, user_data) bind(c)
    ...
    ! resubmit
    pcb => recv_callback
    call ghex_comm_resubmit_recv(communicators(thrid), mesg, rank, tag, pcb, user_data=user_data, req=rrequests(tag))
