...
void test_decomposition_3_4(
    int node_X, int node_Y, int node_Z,
    int numa_X, int numa_Y, int numa_Z,
    ...
    )
{
    ...
    decomp_t decomp(
        ...
        {numa_X, numa_Y, numa_Z},
        {rank_X, rank_Y, rank_Z},
        {thread_X, thread_Y, thread_Z});
    
    // check total number of domains
    EXPECT_EQ(decomp.size(),
        node_X*node_Y*node_Z*numa_X*numa_Y*numa_Z*rank_X*rank_Y*rank_Z*thread_X*thread_Y*thread_Z);

    // check number of nodes
    EXPECT_EQ(decomp.nodes(),
         node_X*node_Y*node_Z);

    // check number of numa nodes per node
    EXPECT_EQ(decomp.numas_per_node(),
         numa_X*numa_Y*numa_Z);

    // check number of ranks per numa node
    EXPECT_EQ(decomp.ranks_per_numa(),
         rank_X*rank_Y*rank_Z);

    // check number of threads per rank
    EXPECT_EQ(decomp.threads_per_rank(),
         thread_X*thread_Y*thread_Z);
    ...
    for (int node_x=0; node_x<node_X; ++node_x)
    {
        ...
        for (int numa_x=0; numa_x<numa_X; ++numa_x)
        {
            ...
            for (int rank_x=0; rank_x<rank_X; ++rank_x)
            {
                ...
                for (int thread_x=0; thread_x<thread_X; ++thread_x)
                {
                    // check level indices
                    ...
                    // check coordinate
                    EXPECT_EQ( decomp(idx)[0], thread_x + thread_X*(rank_x + rank_X*(numa_x + numa_X*(node_x))));
                    EXPECT_EQ( decomp(idx)[1], thread_y + thread_Y*(rank_y + rank_Y*(numa_y + numa_Y*(node_y))));
                    EXPECT_EQ( decomp(idx)[2], thread_z + thread_Z*(rank_z + rank_Z*(numa_z + numa_Z*(node_z))));
