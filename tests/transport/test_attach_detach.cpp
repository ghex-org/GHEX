...
TEST(attach, attach_progress) {
    ...
    cb_comm.attach_recv(std::move(recv_future), recv_msg, src, 0,
                        [&cb_count](const message_type&, int, int) { ++cb_count; });
    ...
    for (unsigned int i = 0; i < SIZE / sizeof(int); ++i) ok = ok && (recv_msg.data<int>()[i] == (int)(i * 10 + src));
...
TEST(detach, detach_cancel_unexpected) {
    ...
    if (auto o = cb_comm.detach_send(dst, 0)) {
        if (!(o->first.ready())) ok = ok && o->first.cancel();
    } else
        ok = false;
