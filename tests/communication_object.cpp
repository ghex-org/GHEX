...
class my_data_desc {
  ...
  public:
    ...
    std::size_t data_type_size() const { return sizeof(T); }
...
TEST(communication_object, constructor) {
    ...
    domain_descriptor_t my_domain_1{
        ...
        coordinate_t{(context.world().rank() % d1 + 1) * DIM1 - 1, (context.world().rank() / d1 + 1) * DIM2 - 1,
                     DIM3 - 1}};
...
TEST(communication_object, exchange) {
    ...
    triple_t<USE_DOUBLE, double>* _values_1 =
        new triple_t<USE_DOUBLE, double>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, double>, layout_map_type> values_1(_values_1, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                  (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, double>, domain_descriptor_t> data_1{local_domains[0],
                                                                           coordinate_t{H1m, H2m, H3m}, values_1};

    /* Just an initialization */
    for (int ii = 0; ii < DIM1 + H1m + H1p; ++ii)
        for (int jj = 0; jj < DIM2 + H2m + H2p; ++jj)
            for (int kk = 0; kk < DIM3 + H3m + H3p; ++kk) { values_1(ii, jj, kk) = triple_t<USE_DOUBLE, double>(); }
...
TEST(communication_object, exchange_multiple_fields) {
    ...
    triple_t<USE_DOUBLE, int>* _values_1 =
        new triple_t<USE_DOUBLE, int>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, int>, layout_map_type> values_1(_values_1, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                               (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, int>, domain_descriptor_t> data_1{local_domains[0], coordinate_t{H1m, H2m, H3m},
                                                                        values_1};

    triple_t<USE_DOUBLE, double>* _values_2 =
        new triple_t<USE_DOUBLE, double>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, double>, layout_map_type> values_2(_values_2, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                  (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, double>, domain_descriptor_t> data_2{local_domains[0],
                                                                           coordinate_t{H1m, H2m, H3m}, values_2};
        ...
        for (int jj = H2m; jj < DIM2 + H2m; ++jj)
            for (int kk = H3m; kk < DIM3 + H3m; ++kk) {
                values_1(ii, jj, kk) = triple_t<USE_DOUBLE, int>(
                    ii - H1m + DIM1 * coords[0], jj - H2m + DIM2 * coords[1], kk - H3m + DIM3 * coords[2]);
                values_2(ii, jj, kk) =
                    triple_t<USE_DOUBLE, double>(ii - H1m + DIM1 * coords[0] + add, jj - H2m + DIM2 * coords[1] + add,
...
TEST(communication_object, multithreading) {
    ...
    domain_descriptor_t my_domain_1{
        ...
        coordinate_t{(context.world().rank() % d1) * DIM1 * 2 + DIM1 - 1, (context.world().rank() / d1 + 1) * DIM2 - 1,
                     DIM3 - 1}};
    local_domains.push_back(my_domain_1);
    ...
    domain_descriptor_t my_domain_2{
        ...
        coordinate_t{(context.world().rank() % d1) * DIM1 * 2 + DIM1 * 2 - 1,
                     (context.world().rank() / d1 + 1) * DIM2 - 1, DIM3 - 1}};
    ...
    array<triple_t<USE_DOUBLE, double>, layout_map_type> values_2(_values_2, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                  (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, double>, domain_descriptor_t> data_2{local_domains[1],
                                                                           coordinate_t{H1m, H2m, H3m}, values_2};
    ...
    for (int ii = H1m; ii < DIM1 + H1m; ++ii)
        for (int jj = H2m; jj < DIM2 + H2m; ++jj)
            for (int kk = H3m; kk < DIM3 + H3m; ++kk) {
                values_1(ii, jj, kk) = triple_t<USE_DOUBLE, double>(
                    ii - H1m + DIM1 * 2 * coords[0], jj - H2m + DIM2 * coords[1], kk - H3m + DIM3 * coords[2]);
                values_2(ii, jj, kk) = triple_t<USE_DOUBLE, double>(
                    ii - H1m + DIM1 * 2 * coords[0] + DIM1, jj - H2m + DIM2 * coords[1], kk - H3m + DIM3 * coords[2]);
            }
...
TEST(communication_object, multithreading_multiple_fileds) {
    ...
    triple_t<USE_DOUBLE, int>* _values_1_1 =
        new triple_t<USE_DOUBLE, int>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, int>, layout_map_type> values_1_1(_values_1_1, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                 (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, int>, domain_descriptor_t> data_1_1{local_domains[0], coordinate_t{H1m, H2m, H3m},
                                                                          values_1_1};

    triple_t<USE_DOUBLE, double>* _values_1_2 =
        new triple_t<USE_DOUBLE, double>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, double>, layout_map_type> values_1_2(_values_1_2, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                    (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, double>, domain_descriptor_t> data_1_2{local_domains[0],
                                                                             coordinate_t{H1m, H2m, H3m}, values_1_2};

    triple_t<USE_DOUBLE, int>* _values_2_1 =
        new triple_t<USE_DOUBLE, int>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, int>, layout_map_type> values_2_1(_values_2_1, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                 (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, int>, domain_descriptor_t> data_2_1{local_domains[1], coordinate_t{H1m, H2m, H3m},
                                                                          values_2_1};

    triple_t<USE_DOUBLE, double>* _values_2_2 =
        new triple_t<USE_DOUBLE, double>[(DIM1 + H1m + H1p) * (DIM2 + H2m + H2p) * (DIM3 + H3m + H3p)];
    array<triple_t<USE_DOUBLE, double>, layout_map_type> values_2_2(_values_2_2, (DIM1 + H1m + H1p), (DIM2 + H2m + H2p),
                                                                    (DIM3 + H3m + H3p));
    my_data_desc<triple_t<USE_DOUBLE, double>, domain_descriptor_t> data_2_2{local_domains[1],
                                                                             coordinate_t{H1m, H2m, H3m}, values_2_2};
